...
registry archive 必须先匹配 `Cargo.lock` checksum，Cargo 再从空 source 目录重新解包。构建环境采用
最小白名单和专属临时 target，guest 环境确保 `VOWORK` 未设置，使 CLI 从 BlockKart 根目录按最近
祖先规则选中受版本控制的 `vo.work`，并执行任务输出目录内原子发布的绝对 `VO_BIN`。构建报告记录当前 Cargo/Rustc/target、CLI 二进制摘要和
execution digest；package、validate、source-audit、regenerate 共同拒绝源码闭包漂移、执行身份缺失、
二进制变化，以及超出限定字段的跨平台差异。

本计划的目标口径：

```text
qualityReady=pending-final-gates
industrialReady=pending-final-gates
architectureEleganceReady=true
sourceFirstPrinciplesReview=pass
artifactSourceAgreement=pending-current-source-regeneration
```
...
- Cargo 构建环境只继承显式主机路径/临时目录字段，固定 locale、时区、颜色和 `VOWORK=off`；
  构建前把 bounded registry cache/index 稳定复制进专属临时 Cargo home，按 `Cargo.lock` 验证所有
  可用锁定 archive，并在专属临时 target 中显式构建 host target。VPAK guest 只接收固定目录、
  任务发布的绝对 `VO_BIN` 与必要 Windows 字段，并明确删除 `VOWORK` 以启用最近祖先工作区发现。
- 所有参与源码闭包的仓库用 `.gitattributes` 固定 LF；VPAK checker 对每个未压缩 archive entry
  验证 portable path、范围、CRC、尺寸以及 payload 与声明 source 的逐字节一致性。
- VPAK build report 绑定当前 CI run、toolchain、CLI binary、CLI build inputs 与 execution digest，
  package 将这些稳定字段传入最终 provenance。
- source-audit 验证 `assets/blockkart.vpak` bytes、manifest entries、producer inputs、upstream outputs。
- regenerate check 比较当前源码重新生成后的 provenance 与 artifact bytes；producer identity 容差
  仅覆盖 voplay build platform，以及 host 确实变化时的 VPAK CLI host/target、binary digest/size 和
  execution digest。同 host 的 CLI binary 必须逐字节一致；CLI source inputs、Cargo/Rustc 版本、VPAK inputs/outputs/upstream/
  archive/manifest/pack 必须保持完全一致。
