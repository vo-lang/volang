...
#[test]
fn dynamic_call_metadata_owns_layout_width_boundaries() {
    for arg_slots in WIDTH_BOUNDARIES {
        for ret_slots in WIDTH_BOUNDARIES {
            ...
            closure_caller.instruction_metadata = vec![
            ...
            verify_module(&closure_module).unwrap_or_else(|err| {
                panic!(
                    "CallClosure args={arg_slots} returns={ret_slots} must verify from metadata: {err}"
                )
            });
            ...
            iface_caller.instruction_metadata = vec![
            ...
            verify_module(&iface_module).unwrap_or_else(|err| {
                panic!(
                    "CallIface args={arg_slots} returns={ret_slots} must verify from metadata: {err}"
                )
            });
        }
    }
...
#[test]
fn call_iface_metadata_preserves_zero_and_wide_method_indices() {
    ...
    verify_module(&module).expect("CallIface metadata must preserve method index 256");
}
...
fn call_extern_and_go_island_metadata_own_argument_width_boundaries() {
    for arg_slots in WIDTH_BOUNDARIES {
        ...
        extern_caller.instruction_metadata = vec![
        ...
        verify_module(&extern_module).unwrap_or_else(|err| {
            panic!("CallExtern args={arg_slots} must verify from metadata: {err}")
        });
        ...
        island_caller.code = vec![
            Instruction::new(Opcode::GoIsland, 0, 1, 2),
            Instruction::new(Opcode::Return, 0, 0, 0),
        ];
        island_caller.instruction_metadata = vec![
            InstructionMetadata::CallLayout {
                arg_layout: vec![SlotType::Value; arg_slots],
                ret_layout: Vec::new(),
            },
        ...
        verify_module(&island_module).unwrap_or_else(|err| {
            panic!("GoIsland args={arg_slots} must verify from metadata: {err}")
        });
    }
}
