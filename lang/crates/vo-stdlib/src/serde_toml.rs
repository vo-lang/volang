...
use super::serde::{FormatReader, FormatWriter, ParsedObject, ParsedValue};
...
fn write_toml_string_content(buf: &mut Vec<u8>, s: &str) {
    for c in s.chars() {
        match c {
            ...
            c => {
                let mut t = [0u8; 4];
                buf.extend_from_slice(c.encode_utf8(&mut t).as_bytes());
            }
        }
    }
}
...
fn parse_toml_value<'a>(
    ...
    match bytes[*pos] {
        ...
        b'+' | b'-' | b'0'..=b'9' => parse_toml_number(input, bytes, pos),
        b'i' => {
            // inf
...
fn parse_toml_multiline_basic_string(
    _input: &str,
    bytes: &[u8],
    pos: &mut usize,
) -> Result<String, &'static str> {
...
fn parse_toml_number<'a>(
    ...
    if *pos < bytes.len() {
        if bytes[*pos] == b'i' && *pos + 2 < bytes.len() && &bytes[*pos..*pos + 3] == b"inf" {
            *pos += 3;
            let val = if input.as_bytes()[start] == b'-' {
                f64::NEG_INFINITY
            } else {
                f64::INFINITY
            };
