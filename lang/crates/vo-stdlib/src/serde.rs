...
fn marshal_fields_into<W: FormatWriter>(
    ...
    for field in &struct_meta.fields {
        ...
        if field.embedded {
            ...
            if !writer.write_field_start(&field_name, *first) {
                continue;
            }
            *first = false;
    ...
    match vk {
        ...
        ValueKind::Struct => {
            ...
            if ptr.is_null() {
                ...
                marshal_struct_value_depth(call, ptr, rttid, writer, depth + 1)?;
            }
        }
        ValueKind::Pointer => {
            let ptr = slot1 as GcRef;
...
fn get_map_key_val_rttids(
    ...
    loop {
        ...
        match rts
            .get(rttid as usize)
            .ok_or("map rttid not found in runtime types")?
        {
...
pub fn get_field_name<'a>(field_name: &'a str, tag: Option<&str>, tag_key: &str) -> Cow<'a, str> {
    ...
    match chars.next() {
        Some(c) if c.is_uppercase() => {
            Cow::Owned(c.to_lowercase().collect::<String>() + chars.as_str())
        }
