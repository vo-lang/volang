    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ReleaseError::RepoRootNotDirectory(path) => {
                write!(f, "repo root does not exist or is not a directory: {}", path.display())
            }
            ...
            ReleaseError::GitError { repo_root, message } => {
                write!(f, "failed to read git commit for {}: {}", repo_root.display(), message)
            }
            ...
            ReleaseError::ForbiddenVoSum { repo_root, paths } => {
                let formatted = paths
                    ...
                    .collect::<Vec<_>>()
                    .join(", ");
                write!(f, "forbidden vo.sum files present in {}: {}", repo_root.display(), formatted)
            }
