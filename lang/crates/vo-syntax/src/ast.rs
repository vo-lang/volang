//! Abstract Syntax Tree definitions for Vo.
//!
...
pub struct FuncDecl {
    ...
    pub sig: FuncSig,
    /// The function body. None means extern function (implemented outside Vo).
...
impl FuncDecl {
    /// Returns true if this is an extern function (no body, implemented outside Vo).
...
pub fn walk_type_expr<V: Visitor>(visitor: &mut V, ty: &TypeExpr) {
    match &ty.kind {
        ...
        TypeExprKind::Port(p) => visitor.visit_type_expr(&p.elem),
        TypeExprKind::Island => {}
        TypeExprKind::Func(f) => {
            for p in &f.params {
