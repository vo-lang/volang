...
fn format_source_once(source: &str) -> Result<String, String> {
    ...
    let canonical = crate::display::format_file(&file, &interner);
    let mut formatted = canonical.clone();
    let inline_mod_anchor = inline_mod_span
        .map(|source_span| {
            preserve_inline_mod_text(source, source_span.clone(), &mut formatted).map(
                |formatted_start| InlineModAnchor {
                    source_span,
                    formatted_start,
                },
            )
        ...
        .transpose()?;
    formatted = restore_comments(source, &formatted, inline_mod_anchor.as_ref())?;
    formatted = normalize_formatted_layout(&formatted);

    let (formatted_file, formatted_diags, formatted_interner) = crate::parser::parse(&formatted, 0);
    ...
    let reparsed_canonical = crate::display::format_file(&formatted_file, &formatted_interner);
...
fn lex_tokens(source: &str) -> Vec<LexToken<'_>> {
    let (tokens, _) = crate::Lexer::new(source, 0).collect_tokens();
    tokens
        .into_iter()
        .filter(|token| token.kind != crate::TokenKind::Eof && token.span.start != token.span.end)
        .map(|token| {
...
fn token_matches(left: &LexToken<'_>, right: &LexToken<'_>) -> bool {
    left.kind == right.kind
        && (!left.kind.is_literal() && left.kind != crate::TokenKind::Ident
...
fn restore_comments(
    source: &str,
    formatted: &str,
    inline_mod_anchor: Option<&InlineModAnchor>,
) -> Result<String, String> {
    let comments: Vec<_> = scan_comments(source)
        .into_iter()
        .filter(|comment| {
            !inline_mod_anchor.is_some_and(|anchor| {
                comment.start >= anchor.source_span.start && comment.end <= anchor.source_span.end
            })
        })
        .collect();
    ...
    for (order, comment) in comments.iter().enumerate() {
        ...
        let next_offset = inline_mod_anchor
            .filter(|anchor| comment.end <= anchor.source_span.start)
            .map(|anchor| anchor.formatted_start)
            .or_else(|| {
                next.and_then(|(index, _)| {
                    mapping[index].map(|mapped| formatted_tokens[mapped].start)
                })
            });
        let restore_unmapped_comma = source_tokens
            ...
            .is_some_and(|(index, token)| {
                token.kind == crate::TokenKind::Comma && mapping[index].is_none()
            });
...
"#;
    ...
    fn preserves_multiple_comments_before_inline_mod_in_order() {
        ...
        let formatted = format_source(&source).expect("format prefixed inline module source");
        let copyright = formatted.find("// Copyright Vo contributors").unwrap();
        let license = formatted
            .find("/* SPDX-License-Identifier: MIT */")
            .unwrap();
        let generated = formatted.find("// Generated notice").unwrap();
        let metadata = formatted.find("/*vo:mod").unwrap();
        assert!(copyright < license && license < generated && generated < metadata);
        assert_eq!(format_source(&formatted).unwrap(), formatted);
    }
...
"#;
        ...
        let (_, diags, _) = crate::parser::parse(&formatted, 0);
...
"#;
        ...
        let (_, diagnostics, _) = crate::parser::parse(&formatted, 0);
