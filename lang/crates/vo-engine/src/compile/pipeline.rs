...
pub(super) fn compile_prepared_project<F: FileSystem>(
    ...
    if let Some(single_file_os) = single_file {
        ...
        return compile_from_single_file_context(engine, fs, ctx, root, file_path, mod_cache);
    }
...
pub(super) fn compile_zip(
    ...
    with_zip_project(zip_path, internal_root, |project| project.compile(engine))
}
...
fn compile_from_single_file_context<F: FileSystem>(
    ...
    PreparedProject::load_memory_prepared(fs, project_context, "no .vo files found")?
        .compile(engine)
}
...
pub(super) fn compile_with_project_snapshot(
    ...
    compile_with_project_snapshot_and_generated_inputs(
        engine,
        context,
        stdlib,
        snapshot,
...
pub(super) fn compile_with_prepared_project_snapshot(
    ...
    load_prepared_project_snapshot(context, stdlib, prepared)?.compile(engine)
}
...
pub(super) fn compile_with_project_snapshot_and_generated_inputs(
    ...
    load_project_from_snapshot(context, stdlib, snapshot, generated_inputs)?.compile(engine)
}
...
mod workspace_source_map_tests {
    ...
    fn source_map(path: &str) -> HashMap<String, PathBuf> {
        HashMap::from([("github.com/acme/widgets".to_string(), PathBuf::from(path))])
    }
    ...
    fn workspace_source_map_accepts_authenticated_path_aliases() {
        ...
        fs.identities
            .insert(PathBuf::from("C:/RUNNER~1/workspace/widgets"), vec![7; 24]);
        fs.identities.insert(
            PathBuf::from("C:/Users/runneradmin/workspace/widgets"),
            vec![7; 24],
        );

        assert!(workspace_source_maps_match(
            &fs,
            &source_map("C:/RUNNER~1/workspace/widgets"),
            &source_map("C:/Users/runneradmin/workspace/widgets"),
        )
        .unwrap());
    }
