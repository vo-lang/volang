//! Cranelift compilation, analysis retention, and executable-memory ownership.
use crate::func_compiler::FunctionCompiler;
use crate::loop_compiler::{CompiledLoop, LoopCompiler};
use crate::*;
...
use crate::helpers::{HelperFuncIds, HelperRefs};

pub(crate) fn configure_required_stack_probes(
...
#[cfg(test)]
pub(crate) fn test_frontend_config() -> cranelift_codegen::isa::TargetFrontendConfig {
...
#[cfg(test)]
pub(crate) unsafe fn invoke_test_jit(
    entry: JitFunc,
    ctx: &mut JitContext,
    frame: &mut [u64],
    ret: &mut [u64],
...
#[inline]
pub(crate) fn function_needs_native_root_frame(func: &FunctionDef) -> bool {
...
pub(crate) struct ModuleJitAnalysis {
    pub(crate) inline_plan: Arc<optimizer::ModuleInlinePlan>,
    pub(crate) entry_eligibility: Arc<[JitFrameEntryEligibility]>,
...
impl ModuleJitAnalysis {
    pub(crate) fn build(
        ...
        // Reserve entry facts before retaining recipes, then derive the facts
        // from exactly the bounded recipes that lowering will consume.
        ...
        let entry_eligibility: Arc<[JitFrameEntryEligibility]> =
            Arc::from(contract::module_frame_entry_eligibility_with_graph(
                module,
                env,
                &graph,
                Some(&inline_plan),
...
impl JitCompiler {
    ...
    pub(crate) fn verify_compile_work_budget(func: &FunctionDef) -> Result<(), JitError> {
    ...
    pub fn compile_loaded_tier(
        ...
        self.compile_loaded_tier_with_feedback(func_id, env, tier, &[])
    }

    /// Optional Island-local target observations affect only guarded lowering.
    /// No feedback is retained in module analysis or compiler environment scope.
    pub fn compile_loaded_tier_with_feedback(
        &mut self,
        func_id: u32,
        env: JitCompileEnv<'_>,
        tier: JitTier,
        ...
        self.compile_bound_tier(func_id, func, vo_module, env, tier, feedback)
    }
    ...
    fn compile_bound_tier(
        ...
        let compile_result = {
            ...
            compiler
                .with_dynamic_feedback(feedback)
