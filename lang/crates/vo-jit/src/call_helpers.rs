...
use vo_runtime::TIME_SLICE;

/// Emit budget check at loop back-edge for time-slice scheduling.
/// 
/// Decrements instruction_budget by `cost`. If budget <= 0, spills variables
/// and returns JitResult::Yield so scheduler can switch to another fiber.
/// 
/// # Arguments
/// - `emitter`: The IR emitter (FunctionCompiler or LoopCompiler)
/// - `cost`: Number of bytecode instructions in the loop body
/// - `resume_pc`: PC to resume at after yield
pub fn emit_budget_check<'a, E: IrEmitter<'a>>(
    emitter: &mut E,
    cost: u32,
    resume_pc: usize,
) {
    ...
    // 1. Load budget from ctx.instruction_budget
    let budget_offset = JitContext::OFFSET_INSTRUCTION_BUDGET;
    let budget = emitter.builder().ins().load(types::I32, MemFlags::trusted(), ctx, budget_offset);
    
    // 2. new_budget = budget - cost
    let cost_val = emitter.builder().ins().iconst(types::I32, cost as i64);
    let new_budget = emitter.builder().ins().isub(budget, cost_val);
    
    // 3. Store new_budget
    emitter.builder().ins().store(MemFlags::trusted(), new_budget, ctx, budget_offset);
    
    // 4. Branch: if new_budget <= 0, yield; else continue
    let yield_block = emitter.builder().create_block();
    let continue_block = emitter.builder().create_block();
    
    let zero = emitter.builder().ins().iconst(types::I32, 0);
    let should_yield = emitter.builder().ins().icmp(IntCC::SignedLessThanOrEqual, new_budget, zero);
    emitter.builder().ins().brif(should_yield, yield_block, &[], continue_block, &[]);
    
    // === Yield block ===
    emitter.builder().switch_to_block(yield_block);
    emitter.builder().seal_block(yield_block);
    
    // Spill all variables so VM can restore them
    emitter.spill_all_vars();
    
    // Reset budget for next time slice
    let reset_val = emitter.builder().ins().iconst(types::I32, TIME_SLICE as i64);
    emitter.builder().ins().store(MemFlags::trusted(), reset_val, ctx, budget_offset);
    
    // Set resume_pc in ctx (reuse call_resume_pc field)
    let resume_pc_offset = JitContext::OFFSET_CALL_RESUME_PC;
    let resume_pc_val = emitter.builder().ins().iconst(types::I32, resume_pc as i64);
    emitter.builder().ins().store(MemFlags::trusted(), resume_pc_val, ctx, resume_pc_offset);
    
    // Return JitResult::Yield = 4
    let yield_result = emitter.builder().ins().iconst(types::I32, 4);
    emitter.builder().ins().return_(&[yield_result]);
