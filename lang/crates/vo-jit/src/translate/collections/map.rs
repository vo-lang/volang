...
pub(in crate::translate) fn map_new<'a>(
    ...
    let func = e.helper(HelperKind::map_new);
    let gc_ptr = e.gc_ptr();
...
pub(in crate::translate) fn map_len<'a>(e: &mut impl CollectionEmitter<'a>, inst: &Instruction) {
    let func = e.helper(HelperKind::map_len);
    let ctx = e.ctx_param();
    let m = e.read_var(inst.b);
    mark_runtime_trap_pc(e);
    let call = emit_runtime_helper_call(e, func, &[ctx, m]);
    let result = e.builder().inst_results(call)[0];
...
fn store_to_stack<'a>(
    ...
    for i in 0..slots {
        ...
        e.builder()
            .ins()
            .stack_store(types::I64, val, stack_slot, (i * 8) as i32);
    }
...
pub(in crate::translate) fn map_get<'a>(
    ...
    e.sync_written_slots(inst.a, layout.val_slots)?;
    if has_ok {
...
pub(in crate::translate) fn map_delete<'a>(
    ...
    let ctx = e.ctx_param();
    mark_runtime_trap_pc(e);
    let call = emit_runtime_helper_call(e, func, &[ctx, m, key_ptr, key_slots_i32]);
    let result = e.builder().inst_results(call)[0];
    emit_return_if_u64_jit_error(e, result);
    let zero = e.builder().ins().iconst(types::I64, 0);
    let is_panic = e.builder().ins().icmp(IntCC::NotEqual, result, zero);
...
pub(in crate::translate) fn map_iter_init<'a>(
    ...
    let ctx = e.ctx_param();
    mark_runtime_trap_pc(e);
    let call = emit_runtime_helper_call(e, func, &[ctx, m, iter_ptr]);
    let result = e.builder().inst_results(call)[0];
    emit_return_if_u64_jit_error(e, result);
    for i in 0..MAP_ITER_SLOTS {
        let val = e
            .builder()
            .ins()
            .stack_load(types::I64, types::I64, iter_slot, (i * 8) as i32);
...
pub(in crate::translate) fn map_iter_next<'a>(
    ...
    for i in 0..MAP_ITER_SLOTS {
        ...
        e.builder()
            .ins()
            .stack_store(types::I64, val, iter_slot, (i * 8) as i32);
    }
    ...
    for i in 0..key_slots {
        let val = e
            .builder()
            .ins()
            .stack_load(types::I64, types::I64, key_slot, (i * 8) as i32);
        e.write_var(inst.a + i as u16, val);
    }
    for i in 0..val_slots {
        let val = e
            .builder()
            .ins()
            .stack_load(types::I64, types::I64, val_slot, (i * 8) as i32);
