use cranelift_codegen::ir::{condcodes::IntCC, types, InstBuilder, MemFlagsData as MemFlags};
...
pub(in crate::translate) fn str_new<'a>(
    ...
    let func = e.helper(HelperKind::str_new_const);
    let const_idx = inst.b as usize;
    let len = match e.vo_module().constants.get(const_idx) {
    ...
    if len == 0 {
        ...
        let ctx = e.ctx_param();
        let index = e.builder().ins().iconst(types::I32, const_idx as i64);
        let call = emit_runtime_helper_call(e, func, &[ctx, index]);
        let result = e.builder().inst_results(call)[0];
        emit_jit_error_if_zero(e, result);
        e.write_var(inst.a, result);
    }
