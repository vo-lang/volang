...
pub use ffi::{
    ...
    AnySlot, InterfaceSlot, ErrorSlot, ClosureCallResult, ClosureCallFn,
};
...
/// Each function must have a corresponding `__STDLIB_<pkg>_<Name>` const generated by `vo_extern_std`.
#[macro_export]
...
pub fn register_all_stdlib_externs(registry: &mut ExternRegistry, externs: &[bytecode::ExternDef]) {
    ...
    // Note: regexp is std-only, not registered in no_std mode yet
}
