...
fn dyn_error(call: &mut ExternCallContext, err: DynErr, fallback_msg: &str) -> ExternResult {
    call.ret_nil(0);
    call.ret_nil(1);
    let kind = match err {
    ...
    if let Some((slot0, slot1)) = dyn_sentinel_error(kind) {
        call.ret_u64(2, slot0);
        call.ret_u64(3, slot1);
    } else {
        write_error_to(call, 2, fallback_msg);
    }
    ExternResult::Ok
}

/// Return a sentinel error for dyn operations (error only, 2 slots).
fn dyn_error_only(call: &mut ExternCallContext, err: DynErr, fallback_msg: &str) -> ExternResult {
    let kind = match err {
        DynErr::NilBase => DynErrorKind::NilBase,
        DynErr::BadField => DynErrorKind::BadField,
        DynErr::BadIndex => DynErrorKind::BadIndex,
    ...
    if let Some((slot0, slot1)) = dyn_sentinel_error(kind) {
        ...
        write_error_to(call, 0, fallback_msg);
    }
    ExternResult::Ok
}
...
fn dyn_call_prepare(call: &mut ExternCallContext) -> ExternResult {
    ...
    macro_rules! return_sentinel_error {
        ($err:expr, $msg:expr) => {{
            ...
            let kind = match $err {
                DynErr::NilBase => Some(DynErrorKind::NilBase),
                DynErr::BadCall => Some(DynErrorKind::BadCall),
                DynErr::SigMismatch => Some(DynErrorKind::SigMismatch),
                DynErr::TypeMismatch => Some(DynErrorKind::TypeMismatch),
                _ => None,
            };
            if let Some((slot0, slot1)) = kind.and_then(dyn_sentinel_error) {
    ...
    if let Err(msg) = call.check_func_signature_compatible(closure_sig_rttid, expected_sig_rttid) {
        return_sentinel_error!(DynErr::SigMismatch, &msg);
    }
