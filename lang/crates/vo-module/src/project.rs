...
use crate::schema::lockfile::{LockFile, LockOrigin, LockedModule, LOCK_FILE_VERSION};
...
pub struct ProjectPlan {
    kind: ProjectPlanKind,
}

#[derive(Debug, Clone, Default)]
enum ProjectPlanKind {
    #[default]
    NoModule,
...
impl ProjectPlan {
    fn no_module() -> Self {
        Self {
            kind: ProjectPlanKind::NoModule,
        }
    }

    fn with_module(context: ProjectModuleContext) -> Self {
        Self {
            kind: ProjectPlanKind::WithModule(Box::new(context)),
        }
    }

    fn module_context(&self) -> Option<&ProjectModuleContext> {
        match &self.kind {
            ProjectPlanKind::NoModule => None,
            ProjectPlanKind::WithModule(context) => Some(context.as_ref()),
        }
    }
    ...
    fn module_context_mut(&mut self) -> Option<&mut ProjectModuleContext> {
        match &mut self.kind {
            ProjectPlanKind::NoModule => None,
            ProjectPlanKind::WithModule(context) => Some(context.as_mut()),
        }
    }
}
...
pub enum ProjectPlanStage {
    Workspace,
    ModFile,
    LockFile,
}

impl ProjectPlanStage {
    fn as_str(self) -> &'static str {
        match self {
            ProjectPlanStage::Workspace => "workspace",
            ProjectPlanStage::ModFile => "mod_file",
            ProjectPlanStage::LockFile => "lock_file",
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProjectPlanErrorKind {
    Missing,
    ReadFailed,
    ParseFailed,
    ValidationFailed,
...
pub type ProjectPlanError = OperationError<ProjectPlanStage, ProjectPlanErrorKind>;

fn project_snapshot_lock_error(project_dir: &Path, error: Error) -> ProjectPlanError {
    ProjectPlanError::new(
        ProjectPlanStage::LockFile,
        ProjectPlanErrorKind::ReadFailed,
...
fn validate_current_toolchain(
    ...
    Err(ProjectPlanError::new(
        ProjectPlanStage::ModFile,
        ProjectPlanErrorKind::ValidationFailed,
        format!(
...
/// Exclusive advisory lock for short recovery and commit phases.
...
pub(crate) fn write_project_files_if_unchanged(
    project_dir: &Path,
    expected_mod: &ModFile,
    expected_lock: Option<&LockFile>,
    next_mod: &ModFile,
    next_lock: Option<&LockFile>,
    operation: &str,
) -> Result<(), Error> {
    let guard = lock_project_mutation(project_dir)?;
    let current_mod = read_mod_file(project_dir)?;
    let current_lock = match read_lock_file(project_dir) {
        Ok(lock) => Some(lock),
        Err(Error::Io(error)) if error.kind() == std::io::ErrorKind::NotFound => None,
        Err(error) => return Err(error),
    };
    if &current_mod != expected_mod || current_lock.as_ref() != expected_lock {
    ...
    write_project_files(project_dir, &guard, next_mod, next_lock)
}

pub(crate) fn remove_lock_for_dependency_free_root(
    project_dir: &Path,
    expected_mod: &ModFile,
    operation: &str,
) -> Result<(), Error> {
    let guard = lock_project_mutation(project_dir)?;
    let current_mod = read_mod_file(project_dir)?;
    if &current_mod != expected_mod || !current_mod.dependencies.is_empty() {
        return Err(Error::DependencyGraph(format!(
            "project declaration changed while {operation} was running; retry against the current vo.mod graph"
        )));
    }
    write_project_files(project_dir, &guard, &current_mod, None)
}
...
pub fn read_project_plan<F: FileSystem>(fs: &F) -> Result<ProjectPlan, ProjectPlanError> {
    const MAX_SNAPSHOT_ATTEMPTS: usize = 8;
    let root = Path::new(".");
    for _ in 0..MAX_SNAPSHOT_ATTEMPTS {
        ...
        if metadata == final_metadata {
            return project_plan_from_root_metadata(&metadata, root, false);
        }
    }
    Err(ProjectPlanError::new(
        ProjectPlanStage::LockFile,
...
pub fn read_project_plan_at_root(dir: &Path) -> Result<ProjectPlan, ProjectPlanError> {
    let fs = RealFs::new(".");
    for _ in 0..8 {
        ...
        if let Some(guard) = read_guard.as_ref() {
            ...
            if metadata == final_metadata {
                return project_plan_from_root_metadata(&metadata, dir, true);
            }
            continue;
        }
...
pub fn read_inline_project_plan(
    vo_mod_content: &str,
    vo_lock_content: Option<&str>,
) -> Result<ProjectPlan, ProjectPlanError> {
    read_inline_project_plan_with(
        vo_mod_content,
        vo_lock_content,
...
/// use [`read_inline_project_plan`]. Ephemeral projects never participate in a
/// workspace or external dependency graph, so `vo.lock` must be absent.
pub fn read_inline_ephemeral_project_plan(
    vo_mod_content: &str,
    vo_lock_content: Option<&str>,
) -> Result<ProjectPlan, ProjectPlanError> {
    read_inline_project_plan_with(
...
fn read_inline_project_plan_with(
    vo_mod_content: &str,
    vo_lock_content: Option<&str>,
    ...
    let mod_file = parse_mod_file(vo_mod_content).map_err(|error| {
        ProjectPlanError::new(
            ProjectPlanStage::ModFile,
            ProjectPlanErrorKind::ParseFailed,
    ...
    read_project_plan_from_mod_file(&fs, mod_file, &[PathBuf::from("vo.lock")])
}
...
pub fn build_lock_file_from_mod_file(
    ...
    let lock_file = LockFile {
        ...
        root: lock::module_intent_digest(mod_file)?,
...
pub fn build_lock_file_for_project_plan(
    project_plan: &ProjectPlan,
    locked_modules: Vec<LockedModule>,
) -> Result<LockFile, Error> {
    let Some(mod_file) = project_plan.mod_file() else {
...
fn read_project_plan_from_mod_file<F: FileSystem>(
    fs: &F,
    mod_file: ModFile,
    ...
    for lock_candidate in lock_candidates {
        let Some(lock_bytes) =
            read_optional_file_bytes(fs, lock_candidate, ProjectPlanStage::LockFile)?
        else {
            continue;
        };
        let lock_content =
            project_metadata_text(&lock_bytes, lock_candidate, ProjectPlanStage::LockFile)?;
        return project_plan_from_captured_metadata(mod_file, Some(lock_content), lock_candidate);
    }
    let lock_path = lock_candidates
        .first()
        .cloned()
        .unwrap_or_else(|| PathBuf::from("vo.lock"));
    project_plan_from_captured_metadata(mod_file, None, &lock_path)
}

fn project_plan_from_captured_metadata(
    mod_file: ModFile,
    ...
    let Some(lock_content) = lock_content else {
        if mod_file.dependencies.is_empty() {
            return Ok(ProjectPlan::with_module(ProjectModuleContext {
                mod_file,
                current_module,
                lock_state: LockState::NoLock,
            }));
        }
        let mut error = ProjectPlanError::new(
            ProjectPlanStage::LockFile,
            ProjectPlanErrorKind::Missing,
    ...
    let locked_modules = lock_file
        .modules
        .iter()
        .filter(|module| module.origin == LockOrigin::Registry)
        .cloned()
        .collect();
    Ok(ProjectPlan::with_module(ProjectModuleContext {
        mod_file,
        current_module,
...
fn read_optional_file_bytes<F: FileSystem>(
...
) -> Result<Option<Vec<u8>>, ProjectPlanError> {
    let limit = if stage == ProjectPlanStage::LockFile {
        crate::MAX_LOCK_FILE_BYTES
    } else {
        vo_common::vfs::MAX_TEXT_FILE_BYTES
    };
    match fs.entry_kind(path).map_err(|error| {
        ...
        found => Err(ProjectPlanError::new(
            stage,
            ProjectPlanErrorKind::ValidationFailed,
            format!(
...
fn find_project_root_in<F: FileSystem>(fs: &F, dir: &Path) -> Result<PathBuf, ProjectPlanError> {
...
pub fn find_project_root(dir: &Path) -> Result<Option<PathBuf>, ProjectPlanError> {
    let fs = RealFs::new(".");
...
fn try_find_project_root_in<F: FileSystem>(
    fs: &F,
    dir: &Path,
) -> Result<Option<PathBuf>, ProjectPlanError> {
...
fn read_root_project_metadata<F: FileSystem>(
    fs: &F,
    dir: &Path,
) -> Result<RootProjectMetadata, ProjectPlanError> {
    ...
    Ok(RootProjectMetadata {
        mod_bytes: read_optional_file_bytes(fs, &mod_path, ProjectPlanStage::ModFile)?,
        lock_bytes: read_optional_file_bytes(fs, &lock_path, ProjectPlanStage::LockFile)?,
    })
}
...
fn project_plan_from_root_metadata(
    ...
    let Some(mod_bytes) = metadata.mod_bytes.as_deref() else {
        ...
        return Ok(ProjectPlan::no_module());
    };
    let mod_content = project_metadata_text(mod_bytes, &mod_path, ProjectPlanStage::ModFile)?;
    ...
    let lock_content = metadata
        .lock_bytes
        .as_deref()
        .map(|bytes| project_metadata_text(bytes, &lock_path, ProjectPlanStage::LockFile))
        .transpose()?;
    project_plan_from_captured_metadata(mod_file, lock_content, &lock_path)
}

fn parse_root_mod_file(
    metadata: &RootProjectMetadata,
    project_root: &Path,
) -> Result<Option<ModFile>, ProjectPlanError> {
    let Some(bytes) = metadata.mod_bytes.as_deref() else {
        return Ok(None);
    };
    let mod_path = root_metadata_path(project_root, "vo.mod");
    let content = project_metadata_text(bytes, &mod_path, ProjectPlanStage::ModFile)?;
...
impl ProjectContext {
    ...
    pub fn project_plan(&self) -> &ProjectPlan {
        &self.project_plan
    }
    ...
    /// `project_plan`, including typed absence of either file.
    ...
    pub fn has_same_root_authority(&self, other: &Self) -> bool {
        self.project_root == other.project_root
            ...
            && self.project_plan.mod_file() == other.project_plan.mod_file()
            && self.project_plan.lock_file() == other.project_plan.lock_file()
    }
    ...
    pub fn into_parts(self) -> (PathBuf, ProjectPlan, HashMap<String, PathBuf>) {
        (self.project_root, self.project_plan, self.workspace_sources)
    }
}
...
/// selects one explicit graph authority. Dependency-bearing projects require
/// one lock for the complete graph; workspace nodes are authorized only when
/// that lock explicitly selects workspace origin.
pub fn load_project_context<F: FileSystem>(
    fs: &F,
    dir: &Path,
) -> Result<ProjectContext, ProjectPlanError> {
    load_project_context_with_options(fs, dir, &ProjectContextOptions::default())
}
    ...
    for attempt in 0..MAX_SNAPSHOT_ATTEMPTS {
        ...
        if final_root == project_root && final_root_metadata == root_metadata && !lock_appeared {
            ...
            let (context, selected_workfile, workspace_candidates) = match snapshot {
                ...
                Err(error) if error.stage == ProjectPlanStage::Workspace => {
...
fn project_workspace_error(error: Error) -> ProjectPlanError {
    project_source_error(ProjectPlanStage::Workspace, error)
}

fn project_source_error_kind(error: &Error) -> ProjectPlanErrorKind {
    match error {
        Error::Io(_) => ProjectPlanErrorKind::ReadFailed,
        Error::SourceRead(error) if error.kind() == std::io::ErrorKind::InvalidData => {
            ProjectPlanErrorKind::ValidationFailed
        }
        Error::SourceRead(_) => ProjectPlanErrorKind::ReadFailed,
        Error::WorkFileParse(_) | Error::SourceScan(_) => ProjectPlanErrorKind::ParseFailed,
        _ => ProjectPlanErrorKind::ValidationFailed,
    }
}

fn project_source_error(stage: ProjectPlanStage, error: Error) -> ProjectPlanError {
    let kind = project_source_error_kind(&error);
    ProjectPlanError::new(stage, kind, error.to_string())
}
...
fn validate_workspace_member_manifest(
    ...
    if mod_file.module.as_str() != source_entry.module.as_str() {
        return Err(Error::DependencyGraph(format!(
...
fn authorize_locked_workspace_sources(
    ...
    let Some(lock_file) = project_plan.lock_file() else {
        return Ok(Vec::new());
    };
    let workspace_nodes = lock_file
        .modules
        .iter()
        .filter(|module| module.origin == LockOrigin::Workspace)
        .map(|module| (module.path.as_str(), module))
        .collect::<BTreeMap<_, _>>();
    let discovered_by_module = discovered
        .iter()
        .map(|member| (member.module.as_str(), member))
        .collect::<BTreeMap<_, _>>();
    let mut authorized = Vec::new();
    for (module_id, locked) in workspace_nodes {
        let source_entry = discovered_by_module.get(module_id).ok_or_else(|| {
            Error::DependencyGraph(format!(
                "vo.lock selects workspace module {module_id}, but the selected vo.work does not provide it"
            ))
        })?;
        ...
        if mod_file.version != locked.version {
            return Err(Error::DependencyGraph(format!(
                "workspace module {module_id} has version {}, while vo.lock selects {}",
                mod_file.version, locked.version
            )));
        }
        let intent = lock::module_intent_digest(mod_file)?;
        if locked.intent.as_ref() != Some(&intent) {
            return Err(Error::DependencyGraph(format!(
                "workspace module {module_id} changed since vo.lock was generated; run `vo work sync`"
            )));
        }
...
> {
    ...
    let mut project_plan = match root_mod.clone() {
        Some(mod_file) => {
            ...
            let lock_content = root_metadata
                ...
                .map(|bytes| project_metadata_text(bytes, &lock_path, ProjectPlanStage::LockFile))
                .transpose()?;
            project_plan_from_captured_metadata(
                mod_file,
                lock_content,
                &normalize_fs_path(&lock_path),
            )?
        }
        None => project_plan_from_root_metadata(root_metadata, &project_root, false)?,
    };
    ...
    let authority = match root_mod.as_ref() {
        ...
        Some(_) if project_plan.lock_file().is_some() => ProjectAuthority::Lock,
    ...
    let authorized_sources = match (root_mod.as_ref(), authority) {
        (Some(root_mod), ProjectAuthority::Lock) => {
            authorize_locked_workspace_sources(root_mod, &project_plan, &workspace_candidates)
        }
        _ => Ok(Vec::new()),
    }
    .map_err(project_workspace_error)?;
    let authorized_modules = authorized_sources
        ...
        .collect::<BTreeSet<_>>();
    project_plan.select_workspace_sources(&authorized_modules);
    ...
    if project_plan.lock_file().is_some() {
...
impl SingleFileSourceGeneration {
    ...
    /// require the complete source/project/workspace selection generation.
    /// Snapshot consumers should also call `matches_source` against the exact
    /// bytes they captured for analysis.
    pub fn validate<F: FileSystem>(&self, fs: &F) -> Result<(), ProjectPlanError> {
        let (_, current) =
...
pub fn load_single_file_context<F: FileSystem>(
    fs: &F,
    file_path: &Path,
) -> Result<SingleFileContext, ProjectPlanError> {
    load_single_file_context_with_options(fs, file_path, &ProjectContextOptions::default())
}
...
pub fn load_single_file_context_with_options_and_generation<F: FileSystem>(
    ...
    file_path: &Path,
    options: &ProjectContextOptions,
) -> Result<(SingleFileContext, SingleFileSourceGeneration), ProjectPlanError> {
...
fn read_single_file_source_stable<F: FileSystem>(
    fs: &F,
    file_path: &Path,
) -> Result<String, ProjectPlanError> {
...
fn single_file_source_generation_changed(file_path: &Path) -> ProjectPlanError {
    ProjectPlanError::new(
        ProjectPlanStage::ModFile,
        ProjectPlanErrorKind::ValidationFailed,
...
fn single_file_source_kept_changing(file_path: &Path) -> ProjectPlanError {
    ProjectPlanError::new(
        ProjectPlanStage::ModFile,
        ProjectPlanErrorKind::ReadFailed,
...
mod tests {
    ...
    use crate::schema::lockfile::{LockOrigin, LockedModule};
    ...
    fn locked_module(path: &str, _dependencies: &[(&str, &str)]) -> LockedModule {
        LockedModule {
    ...
    fn workspace_locked_module(content: &str) -> LockedModule {
        let mod_file = ModFile::parse(content).unwrap();
        LockedModule {
    ...
    fn rendered_lock(
        ...
        if !dependencies.is_empty() {
            ...
            for (module, constraint) in dependencies {
                mod_content.push_str(&format!("{module:?} = {constraint:?}\n"));
            }
        }
    ...
    fn project_root_discovery_propagates_start_normalization_failures() {
        ...
        assert_eq!(error.kind, ProjectPlanErrorKind::ReadFailed);
    ...
    fn project_root_and_metadata_discovery_reject_portable_protocol_aliases() {
        ...
        let error = read_project_plan(&fs).unwrap_err();
        assert_eq!(error.stage, ProjectPlanStage::LockFile);
        assert_eq!(error.kind, ProjectPlanErrorKind::ValidationFailed);
    ...
    #[test]
    fn read_project_plan_reports_missing_lock_when_external_modules_present() {
        let fs = root_project_fs();
        let error = read_project_plan(&fs).unwrap_err();
        assert_eq!(error.stage, ProjectPlanStage::LockFile);
        assert_eq!(error.kind, ProjectPlanErrorKind::Missing);
        assert_eq!(error.path.as_deref(), Some("vo.lock"));
        assert!(error
            .detail
    ...
    #[test]
    fn read_project_plan_requires_lock_for_multiple_direct_dependencies() {
        let mut fs = MemoryFs::new();
        fs.add_file(
            "vo.mod",
        ...
        let error = read_project_plan(&fs)
    ...
    #[test]
    fn read_project_plan_returns_the_complete_locked_graph() {
        let mut fs = root_project_fs();
        fs.add_file("vo.lock", lock_file_for_workspace_source());
        let deps =
            read_project_plan(&fs).unwrap_or_else(|error| panic!("unexpected error: {error}"));
        assert_eq!(
    ...
    fn build_lock_file_requires_complete_root_graph() {
        let lockless =
            ModFile::parse("format = 1\nmodule = \"github.com/acme/lockless\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n").unwrap();
    ...
    #[test]
    fn build_lock_file_accepts_hydrated_closure_nodes_and_rejects_duplicates() {
        let valid = LockFile::parse(&lock_file_for_workspace_source()).unwrap();
        ...
        let closure = build_lock_file_from_mod_file(&rooted, with_orphan).unwrap();
        assert!(closure
            .find(&ModulePath::parse("github.com/acme/orphan").unwrap())
            .is_some());
    ...
    #[test]
    fn read_project_plan_rejects_a_nonempty_lock_for_the_lockless_graph() {
        let mut fs = MemoryFs::new();
        fs.add_file(
            "vo.mod",
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n",
    ...
    #[test]
    fn read_project_plan_rejects_an_empty_lock_for_the_lockless_graph() {
        let mut fs = MemoryFs::new();
        fs.add_file(
            "vo.mod",
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n",
        );
        let root = ModFile::parse(
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n",
        )
        .unwrap();
        fs.add_file(
            ...
            format!(
                "format = {}\nroot = \"{}\"\n",
                crate::schema::lockfile::LOCK_FILE_VERSION,
                crate::lock::module_intent_digest(&root).unwrap()
            ),
        );
    ...
    fn project_context_classifies_oversized_source_as_validation_failure() {
        ...
        assert_eq!(error.stage, ProjectPlanStage::Workspace);
        assert_eq!(error.kind, ProjectPlanErrorKind::ValidationFailed);
    ...
    fn project_context_keeps_workspace_syntax_errors_as_parse_failures() {
        ...
        assert_eq!(error.stage, ProjectPlanStage::Workspace);
        assert_eq!(error.kind, ProjectPlanErrorKind::ParseFailed);
    }
    ...
    fn absolute_directory_without_a_project_cannot_absorb_relative_root_metadata() {
        ...
        assert!(!context.project_plan().has_mod_file());
    ...
    #[test]
    fn workspace_generation_distinguishes_disabled_from_selected_singleton_workspace() {
        let mut fs = MemoryFs::new();
        fs.add_file(
            "project/vo.mod",
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n",
        );
        fs.add_file("project/vo.work", "format = 1\nmembers = [\".\"]\n");
    ...
    fn root_authority_detects_exact_manifest_and_lock_drift() {
        ...
        .unwrap();
        assert_eq!(
            canonical.project_plan().mod_file(),
            equivalent.project_plan().mod_file()
        );
        ...
        changed_lock.modules[0].release = Some(Digest::from_sha256(b"changed-release"));
    ...
    #[test]
    fn read_inline_project_plan_matches_root_file_parsing() {
        let mut fs = root_project_fs();
        fs.add_file("vo.lock", lock_file_for_workspace_source());
        let mod_content = fs.read_file(Path::new("vo.mod")).unwrap();
        let lock_content = fs.read_file(Path::new("vo.lock")).unwrap();

        let deps = read_inline_project_plan(&mod_content, Some(&lock_content)).unwrap();
    ...
    #[test]
    fn inline_project_accepts_local_roots_while_ephemeral_input_stays_local_only() {
        ...
        let ephemeral_mod = concat!(
            "format = 1\nmodule = \"local/script\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n",
        );
        let project_lock = rendered_lock(
            "github.com/acme/app",
            &[("github.com/vo-lang/core", "0.1.0")],
            vec![dependency],
        );

        assert!(read_inline_project_plan(project_mod, Some(&project_lock)).is_ok());
        assert!(read_inline_ephemeral_project_plan(ephemeral_mod, None).is_ok());

        assert!(read_inline_project_plan(ephemeral_mod, None).is_ok());

        let ephemeral_error =
            read_inline_ephemeral_project_plan(project_mod, Some(&project_lock)).unwrap_err();
        assert_eq!(ephemeral_error.stage, ProjectPlanStage::ModFile);
        assert_eq!(ephemeral_error.kind, ProjectPlanErrorKind::ParseFailed);
    }

    #[test]
    fn workspace_source_requires_an_exact_workspace_lock_node() {
        let app_mod = concat!(
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n\n[dependencies]\n",
            "\"github.com/acme/lib\" = \"0.1.0\"\n",
        ...
        fs.add_file("workspace/app/vo.mod", app_mod);
        fs.add_file(
            "workspace/app/main.vo",
            "package main\nimport \"github.com/acme/lib\"\n",
        );
        fs.add_file("workspace/lib/vo.mod", lib_mod);
        ...
        fs.add_file(
            ...
            rendered_lock(
                "github.com/acme/app",
                &[("github.com/acme/lib", "0.1.0")],
                vec![workspace_locked_module(lib_mod)],
            ),
        );
        ...
        assert_eq!(
            context.workspace_sources().get("github.com/acme/lib"),
            Some(&PathBuf::from("workspace/lib"))
        );
    }

    #[test]
    fn workspace_source_intent_drift_requires_work_sync() {
        let app_mod = concat!(
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n\n[dependencies]\n",
            "\"github.com/acme/lib\" = \"0.1.0\"\n",
        ...
        let original_lib =
            "format = 1\nmodule = \"github.com/acme/lib\"\nversion = \"0.1.0\"\nvo = \"0.1.0\"\n";
        let changed_lib = concat!(
            "format = 1\nmodule = \"github.com/acme/lib\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n\n[dependencies]\n",
            "\"github.com/acme/hidden\" = \"0.1.0\"\n",
        );
        let mut fs = MemoryFs::new();
        fs.add_file(
            "workspace/vo.work",
        ...
        fs.add_file("workspace/app/vo.mod", app_mod);
        fs.add_file("workspace/lib/vo.mod", changed_lib);
        fs.add_file(
            "workspace/app/vo.lock",
            rendered_lock(
                "github.com/acme/app",
                &[("github.com/acme/lib", "0.1.0")],
                vec![workspace_locked_module(original_lib)],
            ),
        );

        let error = load_project_context(&fs, Path::new("workspace/app")).unwrap_err();
        assert_eq!(error.stage, ProjectPlanStage::Workspace);
        assert!(error.detail.contains("run `vo work sync`"), "{error}");
    }

    #[test]
    fn registry_lock_nodes_are_never_overlaid_by_workspace_members() {
        let app_mod = concat!(
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n\n[dependencies]\n",
            "\"github.com/acme/lib\" = \"0.1.0\"\n",
        ...
        assert!(context.workspace_modules().is_empty());
        assert_eq!(
            context.project_plan().locked_modules()[0].origin,
            LockOrigin::Registry
        );
    }

    #[test]
    fn workspace_member_imports_must_follow_its_declared_edges() {
        let app_mod = concat!(
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\n",
            "vo = \"0.1.0\"\n\n[dependencies]\n",
            "\"github.com/acme/lib\" = \"0.1.0\"\n",
        ...
        assert!(
            error
                .detail
                .contains("github.com/acme/lib imports github.com/acme/hidden"),
            "{error}"
        );
    }
    ...
    fn single_file_context_retries_when_an_ancestor_manifest_appears() {
        ...
        assert!(matches!(
            ...
            SingleFileContext::Project(ref project)
                if project.project_plan().current_module() == Some("github.com/acme/appeared")
        ));
        generation.validate(&RealFs::new(".")).unwrap();
    }
    ...
    fn single_file_context_retries_when_the_nearest_project_root_relocates() {
        ...
        assert!(matches!(
            context,
            SingleFileContext::Project(ref project)
                if project.project_plan().current_module() == Some("github.com/acme/inner")
    ...
    #[test]
    fn read_project_plan_rejects_incompatible_toolchain() {
        let mut fs = MemoryFs::new();
        fs.add_file(
            "vo.mod",
            "format = 1\nmodule = \"github.com/acme/app\"\nversion = \"0.1.0\"\nvo = \"9.0.0\"\n",
    ...
    fn authored_project_files_are_replaced_atomically() {
        ...
        assert_eq!(read_lock_file(temp.path()).unwrap().root, lock_file.root);
    ...
    fn project_metadata_reads_reject_linked_and_special_entries() {
        ...
        let linked_error = read_project_plan_at_root(&linked_root).unwrap_err();
        assert_eq!(linked_error.stage, ProjectPlanStage::ModFile);
        assert_eq!(linked_error.kind, ProjectPlanErrorKind::ValidationFailed);
        ...
        let fifo_error = read_project_plan_at_root(&fifo_root).unwrap_err();
        assert_eq!(fifo_error.stage, ProjectPlanStage::ModFile);
        assert_eq!(fifo_error.kind, ProjectPlanErrorKind::ValidationFailed);
        ...
        let parent_error = read_project_plan_at_root(&alias_root).unwrap_err();
        assert_eq!(parent_error.stage, ProjectPlanStage::ModFile);
        assert!(
    ...
    fn project_context_retries_an_unlocked_lock_only_change() {
        ...
        new_lock.modules[0].release = Some(expected_release.clone());
        std::fs::write(temp.path().join("vo.lock"), new_lock.render().unwrap()).unwrap();
        resume_tx.send(()).unwrap();

        let context = reader.join().unwrap();
        assert_eq!(
            context.project_plan().locked_modules()[0].release,
    ...
    fn project_context_retries_when_auto_workspace_appears_before_return() {
        ...
        reached_rx
            .recv_timeout(std::time::Duration::from_secs(5))
            .unwrap();
        std::fs::write(root.join("vo.work"), "format = 1\nmembers = [\".\"]\n").unwrap();
        resume_tx.send(()).unwrap();
    ...
    fn project_context_reader_waits_for_pair_commit_guard() {
        ...
        *PROJECT_LOCK_ATTEMPT_HOOK
        ...
        let reader = std::thread::spawn(move || {
            ...
            result_tx
                .send(context.project_plan().current_module().unwrap().to_string())
                .unwrap();
        });
        attempt_rx
            .recv_timeout(std::time::Duration::from_secs(5))
