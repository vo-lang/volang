...
impl CacheMutationLock {
    pub(crate) fn shared(cache_root: &Path) -> Result<Self, Error> {
        Self::acquire(cache_root, false)
    }

    pub(crate) fn exclusive(cache_root: &Path) -> Result<Self, Error> {
        Self::acquire(cache_root, true)
    }
    ...
    fn acquire(cache_root: &Path, exclusive: bool) -> Result<Self, Error> {
        require_secure_cache_mutation_support()?;
        #[cfg(any(target_os = "linux", target_os = "macos"))]
        {
            let cache_root = normalize_cache_root_path(cache_root)?;
            let anchored_root = open_cache_root(&cache_root, !exclusive)?;
            let owner_marker = open_cache_owner_marker(&anchored_root, !exclusive)?;
            let staging = if exclusive {
                anchored_root
...
impl CacheTransaction {
    ...
    /// Atomically replace one existing cache directory with a staged
    /// directory. The displaced directory is moved into this transaction and
    /// removed by transaction cleanup, so readers holding its descriptor can
    /// finish without ever observing a half-written file pair.
    pub(crate) fn replace_directory(
        ...
        #[cfg(any(target_os = "linux", target_os = "macos"))]
        {
            ...
            let (destination_parent_path, destination_name) =
                split_relative_entry(destination, "transaction destination")?;
            let destination_parent = self.root.open_relative_directory(
                destination_parent_path,
                "module cache destination directory",
            )?;
            let displaced_directory = destination_parent
                .open_child(destination_name, "existing module cache destination")?;
            ...
            destination_parent.require_child_identity(
                destination_name,
                &displaced_directory,
                "existing module cache destination",
            )?;
            self.root.require_relative_directory_identity(
                destination_parent_path,
                &destination_parent,
            ...
            exchange_between_directories(
                &source_parent,
                source_name,
                &destination_parent,
                destination_name,
            )?;
            // Until both exchanged names, the configured cache-root location,
            // and parent-directory durability have been confirmed, retain the
            // displaced complete generation in staging. Deleting it on an
            // unconfirmed result would discard the only rollback evidence.
            self.state = CacheTransactionState::ExchangedUnconfirmed;
            ...
            complete_publication(
                ...
                || {
                    destination_parent.require_child_identity(
                        destination_name,
                        &source_directory,
                        "replaced module cache destination",
                    )?;
                    source_parent.require_child_identity(
                        source_name,
                        &displaced_directory,
                        "displaced module cache destination",
                    )
                },
            )?;
            self.state = CacheTransactionState::PublishedEntry;
    ...
    pub(crate) fn cleanup(&mut self) -> Result<(), Error> {
        #[cfg(any(target_os = "linux", target_os = "macos"))]
        {
            match self.state {
                ...
                | CacheTransactionState::ExchangedUnconfirmed => {
...
fn open_cache_owner_marker(root: &AnchoredDirectory, create: bool) -> Result<File, Error> {
    ...
    for _ in 0..OWNER_MARKER_ACQUISITION_ATTEMPTS {
        let (flags, created, adopted_entries) = match root.entry_kind(name)? {
            FileSystemEntryKind::Missing => {
                ...
                if !entries.is_empty() {
                    validate_pre_owner_cache_root(root, &entries)
                        .map_err(|error| pre_owner_cache_migration_error(root, error))?;
                }
                (
                    libc::O_RDWR
                        | libc::O_CLOEXEC
        ...
        if created {
            let initialization = (|| -> Result<(), Error> {
                ...
                file.lock()?;
                #[cfg(test)]
                perform_owner_marker_adoption_race_for_test(root);
                ...
                let expected_entries = adopted_entries.as_ref().ok_or_else(|| {
                    invalid_cache_state(
                        "owner-marker creation lost its pre-owner cache snapshot".to_string(),
                    )
                })?;
                if &entries != expected_entries {
                    return Err(invalid_cache_state(format!(
                        "module cache root {} gained entries or changed existing entries while owner marker {CACHE_OWNER_MARKER} was being created",
                        root.display_path.display(),
                    )));
                }
                if !entries.is_empty() {
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn pre_owner_cache_migration_error(root: &AnchoredDirectory, error: Error) -> Error {
    invalid_cache_state(format!(
        "module cache root {} cannot be safely upgraded to the owner-marker format; clean or relocate the legacy cache and repopulate it: {error}",
        root.display_path.display(),
    ))
}

/// Authenticate a cache created before owner markers were introduced.
///
/// Non-empty roots are adopted only when at least one complete modern cache
/// generation proves the directory's purpose. Every modern entry must be
/// canonical and self-identifying, and every other root entry must be one of
/// the exact sidecars produced by older Volang cache layouts. This keeps the
/// compatibility path from turning a merely non-empty directory into cleanup
/// authority over unrelated user data.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn validate_pre_owner_cache_root(
    root: &AnchoredDirectory,
    entries: &[OsString],
) -> Result<(), Error> {
    let mut authenticated_versions = 0usize;
    for name in entries {
        let path = root.display_path.join(name);
        if let Some(expected_kind) = legacy_cache_sidecar_kind(name) {
            let found = root.entry_kind(name)?;
            if found != expected_kind {
                return Err(invalid_cache_state(format!(
                    "legacy module cache sidecar {} must be {expected_kind:?}; found {found:?}",
                    path.display(),
                )));
            }
            continue;
        ...
        let entry_kind = root.entry_kind(name)?;
        if entry_kind != FileSystemEntryKind::Directory {
            return Err(invalid_cache_state(format!(
                "pre-owner module cache entry {} must be a canonical module directory; found {entry_kind:?}",
                path.display(),
            )));
        }
        let name_text = name.to_str().ok_or_else(|| {
            invalid_cache_state(format!(
                "pre-owner module cache key is not valid UTF-8: {}",
                path.display(),
            ))
        })?;
        let module =
            crate::identity::ModulePath::parse(&name_text.replace('@', "/")).map_err(|error| {
                invalid_cache_state(format!(
                    "pre-owner module cache contains invalid cache key {name_text:?}: {error}",
                ))
            })?;
        let expected_key = crate::cache::layout::cache_key(&module);
        if name_text != expected_key {
            return Err(invalid_cache_state(format!(
                "pre-owner module cache key must be {expected_key:?}, found {name_text:?}",
            )));
        }

        let module_directory = root.open_child(name, "pre-owner module cache key")?;
        let versions = module_directory.entries()?;
        if versions.is_empty() {
            return Err(invalid_cache_state(format!(
                "pre-owner module cache key {} contains no installed versions",
                path.display(),
            )));
        }
        for version_name in versions {
            authenticated_versions = authenticated_versions.checked_add(1).ok_or_else(|| {
                invalid_cache_state(
                    "pre-owner module cache version count overflows usize".to_string(),
                )
            })?;
            if authenticated_versions > MAX_DIRECTORY_ENTRIES {
                return Err(invalid_cache_state(format!(
                    "pre-owner module cache {} contains more than {MAX_DIRECTORY_ENTRIES} installed versions",
                    root.display_path.display(),
                )));
            }
            validate_pre_owner_cache_version(&module_directory, &module, &version_name)?;
        }
    }

    if authenticated_versions == 0 {
        return Err(invalid_cache_state(format!(
            "module cache root {} has no authenticated modern cache entries and cannot be adopted",
            root.display_path.display(),
        )));
    }
    root.require_path_identity(&root.display_path, "pre-owner module cache root")
}

pub(crate) fn legacy_cache_sidecar_kind(name: &OsStr) -> Option<FileSystemEntryKind> {
    if [".cargo-target", ".tmp", "github.com"]
        .into_iter()
        .any(|candidate| name == OsStr::new(candidate))
    {
        Some(FileSystemEntryKind::Directory)
    } else if name == OsStr::new("vo.sum") {
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn validate_pre_owner_cache_version(
    module_directory: &AnchoredDirectory,
    module: &crate::identity::ModulePath,
    version_name: &OsStr,
) -> Result<(), Error> {
    let version_path = module_directory.display_path.join(version_name);
    if module_directory.entry_kind(version_name)? != FileSystemEntryKind::Directory {
        return Err(invalid_cache_state(format!(
            "pre-owner module cache version {} must be a real directory",
            version_path.display(),
        )));
    }
    let version_text = version_name.to_str().ok_or_else(|| {
        invalid_cache_state(format!(
            "pre-owner module cache version is not valid UTF-8: {}",
            version_path.display(),
        ))
    })?;
    let version = crate::version::ExactVersion::parse(version_text).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner module cache contains invalid version directory {}: {error}",
            version_path.display(),
        ))
    })?;
    if version.to_string() != version_text || !module.accepts_version(&version) {
        return Err(invalid_cache_state(format!(
            "pre-owner module cache version {} is not canonical for module {}",
            version_path.display(),
            module,
        )));
    }
    ...
    let version_directory =
        module_directory.open_child(version_name, "pre-owner module cache version")?;
    validate_pre_owner_marker(
        &version_directory,
        crate::cache::layout::VERSION_MARKER,
        version_text,
    )?;
    let source_digest = read_pre_owner_regular_file(
        &version_directory,
        crate::cache::layout::SOURCE_DIGEST_MARKER,
        1024,
    )?;
    let source_digest = std::str::from_utf8(&source_digest).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner source digest marker {} is not valid UTF-8: {error}",
            version_path
                .join(crate::cache::layout::SOURCE_DIGEST_MARKER)
                .display(),
        ))
    })?;
    let source_digest =
        crate::cache::layout::canonical_marker_value(source_digest).ok_or_else(|| {
            invalid_cache_state(format!(
                "pre-owner source digest marker {} is not canonical",
                version_path
                    .join(crate::cache::layout::SOURCE_DIGEST_MARKER)
                    .display(),
            ))
        })?;
    crate::digest::Digest::parse(source_digest).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner source digest marker {} is invalid: {error}",
            version_path
                .join(crate::cache::layout::SOURCE_DIGEST_MARKER)
                .display(),
        ))
    ...
    let mod_bytes = read_pre_owner_regular_file(
        &version_directory,
        "vo.mod",
    ...
    let mod_text = std::str::from_utf8(&mod_bytes).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner cached vo.mod {} is not valid UTF-8: {error}",
            version_path.join("vo.mod").display(),
        ))
    })?;
    let mod_file = crate::schema::modfile::ModFile::parse(mod_text).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner cached vo.mod {} is invalid: {error}",
            version_path.join("vo.mod").display(),
        ))
    })?;
    if mod_file.module.as_github() != Some(module) {
        return Err(invalid_cache_state(format!(
            "pre-owner cached vo.mod {} identifies {}, expected {}",
            version_path.join("vo.mod").display(),
            mod_file.module,
            module,
        )));
    ...
    let release_bytes = read_pre_owner_regular_file(
        &version_directory,
        "vo.release.json",
        vo_common::vfs::MAX_TEXT_FILE_BYTES,
    )?;
    let release_text = std::str::from_utf8(&release_bytes).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner release manifest {} is not valid UTF-8: {error}",
            version_path.join("vo.release.json").display(),
        ))
    })?;
    let release =
        crate::schema::manifest::ReleaseManifest::parse(release_text).map_err(|error| {
            invalid_cache_state(format!(
                "pre-owner release manifest {} is invalid: {error}",
                version_path.join("vo.release.json").display(),
            ))
        })?;
    if &release.module != module || release.version != version {
        return Err(invalid_cache_state(format!(
            "pre-owner release manifest {} does not identify {} {}",
            version_path.join("vo.release.json").display(),
            module,
            version,
        )));
    }
    if release.source.digest.as_str() != source_digest {
        return Err(invalid_cache_state(format!(
            "pre-owner source digest marker {} does not match release source digest {}",
            version_path
                .join(crate::cache::layout::SOURCE_DIGEST_MARKER)
                .display(),
    ...
    if release.vo != mod_file.vo {
        return Err(invalid_cache_state(format!(
            "pre-owner cached vo.mod {} toolchain constraint {} does not match release constraint {}",
            version_path.join("vo.mod").display(),
            mod_file.vo,
            release.vo,
        )));
    }
    let mod_requirements = crate::cache::validate::canonical_mod_requirements(&mod_file.require);
    let release_requirements =
        crate::cache::validate::canonical_manifest_requirements(&release.require);
    if mod_requirements != release_requirements {
        return Err(invalid_cache_state(format!(
            "pre-owner cached vo.mod {} requirements {mod_requirements:?} do not match release requirements {release_requirements:?}",
            version_path.join("vo.mod").display(),
        )));
    }
    Ok(())
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn validate_pre_owner_marker(
    directory: &AnchoredDirectory,
    name: &str,
    expected: &str,
) -> Result<(), Error> {
    let bytes = read_pre_owner_regular_file(directory, name, 1024)?;
    let content = std::str::from_utf8(&bytes).map_err(|error| {
        invalid_cache_state(format!(
            "pre-owner module cache marker {} is not valid UTF-8: {error}",
            directory.display_path.join(name).display(),
        ))
    })?;
    if crate::cache::layout::canonical_marker_value(content) != Some(expected) {
        return Err(invalid_cache_state(format!(
            "pre-owner module cache marker {} must contain {expected:?}",
            directory.display_path.join(name).display(),
        )));
    }
    Ok(())
...
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn read_pre_owner_regular_file(
    directory: &AnchoredDirectory,
    name: &str,
    max_bytes: usize,
) -> Result<Vec<u8>, Error> {
    let name = OsStr::new(name);
    if directory.entry_kind(name)? != FileSystemEntryKind::RegularFile {
        return Err(invalid_cache_state(format!(
            "pre-owner module cache file {} must be a non-linked regular file",
            directory.display_path.join(name).display(),
        )));
    }
    directory.read_file(Path::new(name), max_bytes)
}
...
impl AnchoredDirectory {
    ...
    pub(crate) fn read_file(&self, _relative: &Path, _max_bytes: usize) -> Result<Vec<u8>, Error> {
        Err(unsupported_secure_cache_operation())
    }
...
#[cfg(target_os = "linux")]
fn exchange_between_directories(
    from_parent: &AnchoredDirectory,
    from_name: &OsStr,
    to_parent: &AnchoredDirectory,
    to_name: &OsStr,
    ...
    validate_child_name(from_name)?;
    validate_child_name(to_name)?;
    let from_name = c_string(from_name, "replacement source name")?;
    let to_name = c_string(to_name, "replacement destination name")?;
    #[cfg(test)]
    perform_source_rebind_before_publication_for_test(from_parent, from_name.as_c_str());
    let result = unsafe {
        libc::syscall(
...
#[cfg(all(test, any(target_os = "linux", target_os = "macos")))]
static OWNER_MARKER_ADOPTION_RACES: OnceLock<Mutex<BTreeSet<PathBuf>>> = OnceLock::new();
...
#[cfg(all(test, any(target_os = "linux", target_os = "macos")))]
fn perform_owner_marker_adoption_race_for_test(root: &AnchoredDirectory) {
    let should_race = OWNER_MARKER_ADOPTION_RACES
        .get_or_init(|| Mutex::new(BTreeSet::new()))
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner)
    ...
    if should_race {
        ...
        .expect("test foreign entry must be created during marker adoption");
    }
}
...
mod tests {
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    fn create_pre_owner_cache_version(
        root: &Path,
        module_text: &str,
        version_text: &str,
    ) -> PathBuf {
        let module = ModulePath::parse(module_text).unwrap();
        let version = crate::version::ExactVersion::parse(version_text).unwrap();
        let directory = crate::cache::layout::cache_dir(root, &module, &version);
        std::fs::create_dir_all(&directory).unwrap();
        std::fs::write(
            directory.join(crate::cache::layout::VERSION_MARKER),
            format!("{version_text}\n"),
        )
        .unwrap();
        std::fs::write(
            directory.join(crate::cache::layout::SOURCE_DIGEST_MARKER),
            b"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n",
        )
        .unwrap();
        std::fs::write(
            directory.join("vo.mod"),
            format!("module {module_text}\nvo ^0.1.0\n"),
        )
        .unwrap();
        std::fs::write(
            directory.join("vo.release.json"),
            format!(
                concat!(
                    "{{",
                    "\"schema_version\":1,",
                    ...
                    "\"files_size\":1,",
                    "\"files_digest\":\"sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\"",
                    "}},",
                    "\"web_manifest\":{{",
                    "\"size\":1,",
    ...
    fn maximum_module_identity_uses_a_fixed_length_lock_name() {
        ...
        let _identity_lock = cache_lock
            .identity_lock(&format!("source:{module}@v1.0.0"))
            .unwrap();
    ...
    fn noncache_unowned_root_is_never_blindly_adopted() {
        ...
        assert!(
            error.to_string().contains("canonical module directory"),
            "{error}"
        );
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn authenticated_pre_owner_cache_is_adopted_without_changing_cached_or_legacy_entries() {
        let root = tempfile::tempdir().unwrap();
        let cached = create_pre_owner_cache_version(root.path(), "github.com/acme/lib", "v1.2.3");
        for sidecar in [".cargo-target", ".tmp", "github.com"] {
            let directory = root.path().join(sidecar);
            std::fs::create_dir(&directory).unwrap();
            std::fs::write(directory.join("preserve"), sidecar.as_bytes()).unwrap();
        }
        std::fs::write(root.path().join("vo.sum"), b"preserve legacy sums\n").unwrap();

        let lease = CacheMutationLock::shared(root.path()).unwrap();

        lease.validate_cache_ownership().unwrap();
        assert_eq!(
            std::fs::read(cached.join(crate::cache::layout::VERSION_MARKER)).unwrap(),
            b"v1.2.3\n",
        );
        for sidecar in [".cargo-target", ".tmp", "github.com"] {
            assert_eq!(
                std::fs::read(root.path().join(sidecar).join("preserve")).unwrap(),
                sidecar.as_bytes(),
            );
        }
        assert_eq!(
            std::fs::read(root.path().join("vo.sum")).unwrap(),
            b"preserve legacy sums\n",
        );
        assert_eq!(
        ...
        assert_eq!(
            exact_entry_kind(&staging_lock_path(root.path())).unwrap(),
            FileSystemEntryKind::RegularFile,
        );
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn pre_owner_root_with_only_legacy_sidecars_is_not_adopted() {
        let root = tempfile::tempdir().unwrap();
        std::fs::create_dir(root.path().join(".cargo-target")).unwrap();
        std::fs::create_dir(root.path().join(".tmp")).unwrap();
        std::fs::create_dir(root.path().join("github.com")).unwrap();
        std::fs::write(root.path().join("vo.sum"), b"legacy\n").unwrap();

        let error = CacheMutationLock::shared(root.path()).unwrap_err();

        assert!(error.to_string().contains("no authenticated"), "{error}");
        assert!(!root.path().join(CACHE_OWNER_MARKER).exists());
        assert!(!root.path().join(STAGING_DIR).exists());
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn authenticated_pre_owner_cache_with_unknown_root_entry_is_not_adopted() {
        let root = tempfile::tempdir().unwrap();
        create_pre_owner_cache_version(root.path(), "github.com/acme/lib", "v1.2.3");
        std::fs::write(root.path().join("unrelated.txt"), b"preserve").unwrap();
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn pre_owner_cache_metadata_mismatches_are_rejected_without_scaffolding() {
        for mismatch in [
            "version",
            "digest",
            "source-binding",
            ...
            match mismatch {
                "version" => std::fs::write(
                    cached.join(crate::cache::layout::VERSION_MARKER),
                    b"v1.2.4\n",
                )
                .unwrap(),
                "digest" => std::fs::write(
                    cached.join(crate::cache::layout::SOURCE_DIGEST_MARKER),
                    b"sha256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n",
                )
                .unwrap(),
                "source-binding" => std::fs::write(
                    cached.join(crate::cache::layout::SOURCE_DIGEST_MARKER),
                ...
                "mod" => std::fs::write(
                    cached.join("vo.mod"),
                    b"module github.com/acme/other\nvo ^0.1.0\n",
                )
                .unwrap(),
                "vo" => std::fs::write(
                    cached.join("vo.mod"),
                    b"module github.com/acme/lib\nvo ^0.2.0\n",
                )
                .unwrap(),
                "require" => std::fs::write(
                    cached.join("vo.mod"),
                    b"module github.com/acme/lib\nvo ^0.1.0\n\nrequire github.com/acme/dep ^1.0.0\n",
                )
                .unwrap(),
                "release" => std::fs::write(
                    cached.join("vo.release.json"),
                    b"{\"schema_version\":1,\"module\":\"github.com/acme/lib\",\"version\":\"v1.2.4\"}\n",
                )
                .unwrap(),
                "release-contract" => {
                    let path = cached.join("vo.release.json");
                    let release = std::fs::read_to_string(&path).unwrap().replace(
            ...
            assert!(matches!(error, Error::Io(_)), "{mismatch}: {error}");
            assert!(!root.path().join(CACHE_OWNER_MARKER).exists(), "{mismatch}");
            assert!(!root.path().join(STAGING_DIR).exists(), "{mismatch}");
        }
    }

    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn pre_owner_cache_rejects_symlink_and_special_root_entries_without_scaffolding() {
        use std::os::unix::ffi::OsStrExt;
        use std::os::unix::fs::symlink;

        for unsafe_kind in ["symlink", "special"] {
            let root = tempfile::tempdir().unwrap();
            create_pre_owner_cache_version(root.path(), "github.com/acme/lib", "v1.2.3");
            let unsafe_entry = root.path().join(match unsafe_kind {
                "symlink" => ".tmp",
                "special" => ".cargo-target",
                _ => unreachable!(),
            });
            let outside = tempfile::tempdir().unwrap();
            match unsafe_kind {
                "symlink" => {
                    symlink(outside.path(), &unsafe_entry).unwrap();
                }
                "special" => {
                    let path = std::ffi::CString::new(unsafe_entry.as_os_str().as_bytes()).unwrap();
                    let result = unsafe { libc::mkfifo(path.as_ptr(), 0o600) };
                    assert_eq!(
                        result,
                        0,
                        "failed to create FIFO fixture at {}: {}",
                        unsafe_entry.display(),
                        io::Error::last_os_error(),
                    );
                }
                _ => unreachable!(),
            ...
            assert!(error.to_string().contains(match unsafe_kind {
                "symlink" => "Symlink",
                "special" => "Special",
                _ => unreachable!(),
            }));
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn pre_owner_cache_change_during_validation_rolls_back_marker_without_scaffolding() {
        let root = tempfile::tempdir().unwrap();
        let cached = create_pre_owner_cache_version(root.path(), "github.com/acme/lib", "v1.2.3");
        race_owner_marker_adoption_for_test(root.path());
        ...
        assert!(
            error
                .to_string()
                .contains("gained entries or changed existing entries"),
            "{error}"
        );
        assert_eq!(
            std::fs::read(cached.join(crate::cache::layout::VERSION_MARKER)).unwrap(),
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn adopted_legacy_sidecars_clean_without_following_nested_symlinks() {
        use std::os::unix::fs::symlink;

        let root = tempfile::tempdir().unwrap();
        create_pre_owner_cache_version(root.path(), "github.com/acme/lib", "v1.2.3");
        for sidecar in [".cargo-target", ".tmp", "github.com"] {
            std::fs::create_dir(root.path().join(sidecar)).unwrap();
        }
        std::fs::write(root.path().join("vo.sum"), b"legacy\n").unwrap();
        let outside = tempfile::tempdir().unwrap();
        std::fs::write(outside.path().join("preserve"), b"outside").unwrap();
        symlink(
            outside.path(),
            root.path().join(".tmp").join("outside-link"),
        )
        .unwrap();
        drop(CacheMutationLock::shared(root.path()).unwrap());

        let removed = crate::lifecycle::clean_cache(root.path(), None).unwrap();

        assert_eq!(removed, 1);
        for sidecar in [".cargo-target", ".tmp", "github.com"] {
            assert!(!root.path().join(sidecar).exists(), "{sidecar}");
        }
        assert!(!root.path().join("vo.sum").exists());
        assert_eq!(
            std::fs::read(outside.path().join("preserve")).unwrap(),
            b"outside",
        );
        assert!(root.path().join(CACHE_OWNER_MARKER).is_file());
        assert!(staging_lock_path(root.path()).is_file());
    }

    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn cleanup_rejects_legacy_sidecar_type_replacement_without_following_it() {
        use std::os::unix::fs::symlink;
        ...
        let error = crate::lifecycle::clean_cache(root.path(), None).unwrap_err();

        assert!(error.to_string().contains("found Symlink"), "{error}");
        assert_eq!(
            std::fs::read(outside.path().join("preserve")).unwrap(),
            b"outside",
        );
        assert!(cached.is_dir());
        assert!(root.path().join(".tmp").is_symlink());
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn marker_creation_refuses_a_concurrent_root_adoption() {
        let root = tempfile::tempdir().unwrap();
        race_owner_marker_adoption_for_test(root.path());
    ...
    fn identity_lock_rejects_hard_links_without_self_deadlocking() {
        let root = tempfile::tempdir().unwrap();
        let cache_lock = CacheMutationLock::shared(root.path()).unwrap();
        let identity = "source:github.com/acme/lib@v1.0.0";
        let digest = Digest::from_sha256(identity.as_bytes());
        let locks_dir = root.path().join(STAGING_DIR).join("locks");
    ...
    fn identity_lock_keeps_the_shared_lease_after_its_origin_guard_is_dropped() {
        ...
        let identity_lock = cache_lock
            .identity_lock("source:github.com/acme/lib@v1.0.0")
            .unwrap();
        drop(cache_lock);
    ...
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    #[test]
    fn unconfirmed_directory_exchange_retains_the_displaced_generation() {
        let root = tempfile::tempdir().unwrap();
        let cache_lock = CacheMutationLock::shared(root.path()).unwrap();
        cache_lock
            .ensure_directory(Path::new("ephemeral/cache-key"))
            .unwrap();
        std::fs::write(
            root.path().join("ephemeral/cache-key/generation"),
            b"previous",
        )
        .unwrap();

        let mut transaction = cache_lock
            .begin_transaction("exchange-sync-failure")
            .unwrap();
        transaction.create_dir_all(Path::new("entry")).unwrap();
        transaction
            .write_file(Path::new("entry/generation"), b"replacement")
            .unwrap();
        let transaction_path = root.path().join(transaction.relative_path());
        let destination = Path::new("ephemeral/cache-key");
        fail_publication_sync_for_test(&root.path().join(destination));

        let error = transaction
            .replace_directory(Path::new("entry"), destination)
            .unwrap_err();

        assert!(matches!(
            error,
            Error::CachePublicationDurabilityUnconfirmed { ref path, .. }
                if path == "ephemeral/cache-key"
        ));
        drop(transaction);
        assert_eq!(
            std::fs::read(root.path().join(destination).join("generation")).unwrap(),
            b"replacement",
        );
        assert_eq!(
            std::fs::read(transaction_path.join("entry/generation")).unwrap(),
            b"previous",
        );
    }
