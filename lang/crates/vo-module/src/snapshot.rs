...
pub const PROJECT_SNAPSHOT_SCHEMA_VERSION: u64 = 2;
...
impl ProjectSnapshot {
    ...
    fn from_project_context(
        ...
        let project_deps = context.project_deps();
        let mod_file = project_deps.mod_file().cloned().ok_or_else(|| {
            Error::ModFileParse("project snapshot requires a vo.mod project".to_string())
        })?;

        let mut modules = match options.mode {
            GraphMode::Declared => declared_modules(project_deps.lock_file(), cache_root)?,
        ...
        let snapshot = Self {
            ...
            authority: match context.authority() {
                ...
                crate::project::ProjectAuthority::Workspace => SnapshotAuthority::Workspace,
            },
            root: SnapshotRoot {
    ...
    pub fn render(&self) -> Result<Vec<u8>, Error> {
        ...
        string!(match self.authority {
            ...
            SnapshotAuthority::Workspace => "workspace",
        });
        raw!(",\n  \"root\": {\n    \"module\": ");
    ...
    pub fn validate(&self) -> Result<(), Error> {
        if self.schema_version != PROJECT_SNAPSHOT_SCHEMA_VERSION {
            return Err(Error::LockFileParse(format!(
                "unsupported project snapshot schema version: {}",
                self.schema_version
            )));
        }
        ...
        if root_module.as_github().is_none() {
            return Err(Error::LockFileParse(
                "project snapshot root must be a canonical github.com module".to_string(),
            ));
        }
        ...
        match self.authority {
            ...
            SnapshotAuthority::Workspace
                if self.mode != GraphMode::Effective
                    || self.workspace.is_none()
                    || self.root.dependencies.is_empty()
                    || self.modules.is_empty() =>
            {
                return Err(Error::DependencyGraph(
                    "workspace snapshot authority requires an effective, selected, non-empty workspace graph"
                        .to_string(),
                ));
            }
            _ => {}
        ...
        for module in &self.modules {
            ...
            if root_module.as_github() == Some(&path) {
                return Err(Error::LockFileParse(format!(
        ...
        let modules_by_path = self
            ...
            .map(|module| (module.module.as_str(), module))
            .collect::<BTreeMap<_, _>>();
        let mut incoming_constraints = (self.authority == SnapshotAuthority::Workspace)
            .then(BTreeMap::<String, Vec<(String, crate::version::DepConstraint)>>::new);
        ...
        for dependency in &self.root.dependencies {
            ...
            if let Some(incoming_constraints) = incoming_constraints.as_mut() {
                incoming_constraints
                    .entry(dependency.module.clone())
                    .or_default()
                    .push((
                        self.root.module.clone(),
                        crate::version::DepConstraint::parse(&dependency.constraint)?,
                    ));
            }
        }
        for module in &self.modules {
        ...
        if let Some(incoming_constraints) = &incoming_constraints {
            for (module, constraints) in incoming_constraints {
                ...
                return Err(Error::ConflictingConstraints {
                    module: module.clone(),
                    detail: crate::project::summarize_incoming_constraints(constraints),
                });
            }
        }
...
fn validate_edge_selection(
    ...
    if authority == SnapshotAuthority::Workspace {
        return Ok(());
    }
...
fn effective_modules(
    ...
    match context.authority() {
        ...
        crate::project::ProjectAuthority::Lock => effective_locked_modules(
            context.project_deps().lock_file().ok_or_else(|| {
                Error::DependencyGraph("lock authority is missing its captured vo.lock".to_string())
            })?,
            context.workspace_modules(),
            cache_root,
        ),
        crate::project::ProjectAuthority::Workspace => context
            .workspace_modules()
            .iter()
            .map(workspace_authority_snapshot_module)
            .collect(),
    }
}
...
fn workspace_snapshot_module(
    ...
    Ok(SnapshotModule {
        ...
        vo: locked.vo.to_string(),
        release: Some(locked.release.to_string()),
        source: SnapshotSource::Workspace {
            directory: utf8_path(workspace.directory(), "workspace member")?,
        },
        dependencies: locked_dependencies(&locked.dependencies),
    })
}

fn workspace_authority_snapshot_module(
    workspace: &crate::project::WorkspaceModule,
) -> Result<SnapshotModule, Error> {
    Ok(SnapshotModule {
        module: workspace.module().as_str().to_string(),
        version: None,
...
fn locked_dependencies(dependencies: &[LockedDependency]) -> Vec<SnapshotDependency> {
    let mut dependencies = dependencies
        .iter()
        .map(|dependency| SnapshotDependency {
            module: dependency.module.as_str().to_string(),
...
fn validate_module_selection(
    ...
    match authority {
        SnapshotAuthority::Lock => {
            let (Some(version), Some(release)) =
                (selected.version.as_deref(), selected.release.as_deref())
            else {
                return Err(Error::LockFileParse(format!(
                    "lock snapshot module {module} requires version and release"
                )));
            };
        ...
        SnapshotAuthority::Workspace => {
            if selected.version.is_some() || selected.release.is_some() {
                return Err(Error::LockFileParse(format!(
                    "workspace snapshot module {module} must omit version and release"
                )));
            }
            Ok(())
        }
...
"#;
        let error = ProjectSnapshot::parse(legacy_v1).unwrap_err();
        assert!(
            error
                .to_string()
                .contains("unsupported project snapshot schema version: 1"),
            "{error}"
        );
    ...
    fn deserializer_rejects_oversized_collections_before_semantic_validation() {
        ...
        let snapshot = format!(
            concat!(
                r#"{{"schema_version":2,"mode":"declared","authority":"lock","root":{{"module":"github.com/acme/app","vo":"^0.1.0","dependencies":[]}},"modules":{} }}"#,
            ),
    ...
    #[test]
    fn workspace_snapshot_schema_requires_local_sources_and_omitted_lock_fields() {
        let mut snapshot = ProjectSnapshot {
            schema_version: PROJECT_SNAPSHOT_SCHEMA_VERSION,
            mode: GraphMode::Effective,
            authority: SnapshotAuthority::Workspace,
            root: SnapshotRoot {
                module: "github.com/acme/app".into(),
                vo: "^0.2.0".into(),
                dependencies: vec![SnapshotDependency {
        ...
        snapshot.modules[0].version = Some("1.0.0".into());
        assert!(snapshot
            .validate()
            .unwrap_err()
            .to_string()
            .contains("must omit version and release"));
        snapshot.modules[0].version = None;
        snapshot.modules[0].source = SnapshotSource::Registry {
        ...
        assert!(snapshot
            ...
            .unwrap_err()
            .to_string()
            .contains("requires workspace source"));
    }
    ...
    fn workspace_snapshot_schema_rejects_disjoint_incoming_constraints() {
        ...
        assert!(
            error.to_string().contains("conflicting constraints"),
            "{error}"
        );
    }

    #[test]
    fn effective_snapshot_exports_a_closed_workspace_graph_without_a_lock() {
        ...
        .unwrap();
        assert_eq!(snapshot.authority, SnapshotAuthority::Workspace);
        assert_eq!(snapshot.modules.len(), 2);
        assert!(snapshot
            .modules
            .iter()
            .all(|module| module.version.is_none() && module.release.is_none()));
        assert!(snapshot
            .modules
            .iter()
            .all(|module| matches!(module.source, SnapshotSource::Workspace { .. })));
        assert_eq!(
        ...
        let rendered = snapshot.render().unwrap();
        assert_eq!(ProjectSnapshot::parse(&rendered).unwrap(), snapshot);
        assert!(!String::from_utf8(rendered).unwrap().contains("\"version\""));
        assert!(!render_graph(&snapshot).unwrap().contains('@'));
        assert!(!app.join(".vo-project.lock").exists());
        assert!(!a.join(".vo-project.lock").exists());
        assert!(!b.join(".vo-project.lock").exists());
    }

    #[cfg(any(unix, windows))]
    #[test]
    fn effective_snapshot_retries_when_workspace_authority_changes_mid_capture() {
        use std::sync::mpsc;

        let root = tempfile::tempdir().unwrap();
        let root_path = root.path().canonicalize().unwrap();
        let app = root_path.join("app");
        let library = root_path.join("lib");
        ...
        .unwrap();
        std::fs::write(library.join("lib.vo"), "package lib\n").unwrap();
        let lock = crate::schema::lockfile::LockFile {
            ...
            root: crate::schema::lockfile::LockRoot {
                module: crate::identity::ModIdentity::parse("github.com/acme/app").unwrap(),
                vo: crate::version::ToolchainConstraint::parse("^0.1.0").unwrap(),
            },
            modules: vec![locked_module("github.com/acme/lib", "1.0.0", vec![])],
        };
        std::fs::write(app.join("vo.lock"), lock.render().unwrap()).unwrap();

        let (reached_tx, reached_rx) = mpsc::channel();
        let (resume_tx, resume_rx) = mpsc::channel();
        assert!(PROJECT_SNAPSHOT_CONTEXT_PAUSES
            .get_or_init(|| Mutex::new(BTreeMap::new()))
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .insert(
                app.clone(),
                ProjectSnapshotPause {
                    reached: reached_tx,
                    resume: resume_rx,
                },
            )
        ...
        reached_rx
            ...
            .unwrap();
        std::fs::write(root_path.join("vo.work"), "version = 1\nmembers = []\n").unwrap();
    ...
    #[cfg(any(unix, windows))]
    #[test]
    fn effective_snapshot_retries_when_a_member_manifest_changes_mid_capture() {
        use std::sync::mpsc;
        ...
        let app = root_path.join("app");
        let library = root_path.join("lib");
        let utility = root_path.join("util");
        ...
        .unwrap();
        let library_manifest = |constraint: &str| {
            format!(
                "module = \"github.com/acme/lib\"\nvo = \"^0.1.0\"\n[dependencies]\n\"github.com/acme/util\" = \"{constraint}\"\n"
            )
        };
        std::fs::write(library.join("vo.mod"), library_manifest("^1.0.0")).unwrap();
        std::fs::write(
            utility.join("vo.mod"),
        ...
        let reader_app = app.clone();
        let reader_cache = root_path.join("cache");
        let reader = std::thread::spawn(move || {
            ProjectSnapshot::capture(
                &reader_app,
                &reader_cache,
        ...
        reached_rx
            .recv_timeout(std::time::Duration::from_secs(5))
            .unwrap();
        std::fs::write(library.join("vo.mod"), library_manifest("~1.1.0")).unwrap();
        resume_tx.send(()).unwrap();
        ...
        let library = snapshot
            .modules
            .iter()
            .find(|module| module.module == "github.com/acme/lib")
            .unwrap();
        assert_eq!(library.dependencies[0].constraint, "~1.1.0");
        assert!(library.version.is_none());
        assert!(library.release.is_none());
    }

    #[test]
    fn effective_snapshot_uses_locked_edges_when_workspace_constraints_drift() {
        let root = tempfile::tempdir().unwrap();
        let root_path = std::fs::canonicalize(root.path()).unwrap();
        let app = root_path.join("app");
        let a = root_path.join("a");
        ...
        let lock = crate::schema::lockfile::LockFile {
            ...
            modules: vec![
                locked_module(
                    "github.com/acme/a",
                    "1.0.0",
                    vec![("github.com/acme/c", "^1.0.0")],
                ),
                locked_module("github.com/acme/c", "1.0.0", vec![]),
            ],
        };
        std::fs::write(app.join("vo.lock"), lock.render().unwrap()).unwrap();
        ...
        assert!(matches!(
            snapshot.modules[1].source,
            SnapshotSource::Registry { .. }
        ));
        assert_eq!(snapshot.modules[1].version.as_deref(), Some("1.0.0"));
        assert_eq!(
            snapshot.modules[0].dependencies[0].module,
            "github.com/acme/c"
        );
        assert_eq!(snapshot.modules[0].dependencies[0].constraint, "^1.0.0");
        assert_eq!(
    ...
    fn locked_module(
        ...
        crate::schema::lockfile::LockedModule {
            path: crate::identity::ModulePath::parse(module).unwrap(),
            version: crate::version::ExactVersion::parse(version).unwrap(),
            vo: crate::version::ToolchainConstraint::parse("^0.1.0").unwrap(),
            release: crate::digest::Digest::from_sha256(format!("{module}@{version}").as_bytes()),
            dependencies: dependencies
                .into_iter()
                .map(
                    |(module, constraint)| crate::schema::lockfile::LockedDependency {
                        module: crate::identity::ModulePath::parse(module).unwrap(),
                        constraint: crate::version::DepConstraint::parse(constraint).unwrap(),
                    },
                )
                .collect(),
        }
