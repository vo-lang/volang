...
use vo_analysis::vfs::prepare_file_set_with_synthesized_ephemeral_module;
use vo_analysis::vfs::{
...
mod diagnostics;
mod editor;
use diagnostics::{CompileOutput, WebCompileError};
...
fn prepare_analysis_with_package_resolver<R: vo_analysis::vfs::Resolver>(
    input: PreparedCompileInput,
    package_resolver: R,
) -> Result<vo_analysis::vfs::PreparedAnalysis<R, MemoryFs>, WebCompileError> {
    let current_module = input.project_plan.current_module().map(str::to_string);
    let local_fs = input.local_fs;
    match input.module_authority {
        ModuleAnalysisAuthority::Project { current_package } => {
            prepare_file_set_with_package_identity(
                input.file_set,
                package_resolver,
                local_fs,
                input.project_root,
...
fn analyze_with_package_resolver<R: vo_analysis::vfs::Resolver>(
    ...
    let project = prepare_analysis_with_package_resolver(input, package_resolver)?
        .check()
        .map_err(WebCompileError::Analysis)?;
    let imported_packages = project
        ...
        .map(|(path, _, _, _)| path)
        .collect::<Vec<_>>();
    validate_ready_imports(&imported_packages, ready_modules)?;
    Ok(project)
}

fn validate_ready_imports(
    imported_packages: &[&str],
    ready_modules: &[ReadyModule],
) -> Result<(), WebCompileError> {
...
fn compile_with_package_resolver<R: vo_analysis::vfs::Resolver>(
    ...
    package_resolver: R,
    ready_modules: &[ReadyModule],
) -> Result<CompileOutput<Vec<u8>>, WebCompileError> {
    ...
    let bytecode = module.serialize().map_err(|error| {
        WebCompileError::new(
            WebCompileStage::Codegen,
            WebCompileErrorKind::Codegen,
    ...
    Ok(CompileOutput::from_project(bytecode, &project))
}
...
fn check_with_fs_modules<M: FileSystem + Send + Sync>(
    ...
    std_fs: MemoryFs,
    mod_fs: M,
) -> Result<CompileOutput<()>, WebCompileError> {
    validate_materialized_graph(&input, &mod_fs)?;
    let ready_modules = vo_module::readiness::check_materialized_modules_readiness(
        &mod_fs,
        input.project_plan.locked_modules(),
...
fn check_with_ready_fs_modules<M: FileSystem + Send + Sync>(
    ...
    analyze_with_package_resolver(input, package_resolver, ready_modules)
        .map(|project| CompileOutput::from_project((), &project))
}
...
fn compile_entry_with_external_fs_with_options<M: FileSystem + Send + Sync>(
    ...
    compile_with_fs_modules(input, std_fs, mod_fs)
}
...
fn check_entry_with_external_fs_with_options<M: FileSystem + Send + Sync>(
    ...
    let input = prepare_entry_input_with_options(entry, local_fs, options)?;
    check_with_fs_modules(input, std_fs, mod_fs)
}
...
pub fn compile(source: &str, filename: Option<String>) -> CompileResult {
    ...
    diagnostics::result(
        compile_source_detailed(source, &filename, build_stdlib_fs())
...
pub fn compile_project(
    ...
    diagnostics::result(result.map(|output| output.map(Some)))
}
...
pub fn analyze_project(
    ...
    diagnostics::result(result.map(|output| output.map(|_| None)))
}
...
pub async fn compile_project_auto_install(
    ...
    .await;
    Ok(diagnostics::result(compile.map(|output| output.map(Some))))
}
...
pub async fn analyze_project_auto_install(
    ...
    Ok(diagnostics::result(
        analysis.map(|output| output.map(|_| None)),
    ))
}
...
pub fn compile_source_with_std_fs(
    ...
    compile_source_detailed(source, filename, std_fs)
        .map(|output| output.value)
        .map_err(|error| error.to_string())
}

fn compile_source_detailed(
    source: &str,
    filename: &str,
    std_fs: MemoryFs,
) -> Result<CompileOutput<Vec<u8>>, WebCompileError> {
    let input = prepare_single_file_input(source, filename)?;
    compile_with_fs_modules(input, std_fs, MemoryFs::new())
}
