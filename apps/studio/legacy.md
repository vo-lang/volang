# Previous Studio implementation

The Web-first replacement is being implemented in [next/](next/README.md), using
the new UI framework. It has Gallery, Playground and Docs; Git and accounts are
excluded from its scope. The remainder of this document describes the existing
distribution, which remains available during migration.
...
cargo build -p vo-studio-aot-runtime-native --profile release-native
vo ui package apps/studio -o dist/studio-native --runtime=target/release-native/libvo_studio_aot_runtime_native.a
```

The Web release gate builds the complete Wasm VM Studio and drives a
