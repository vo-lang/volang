...
self.onmessage = async event => {
  ...
  try {
    ...
    try {
      ...
      if (!compiled.success) {
        self.postMessage({output:compiled.errorMessage || 'Could not compile this program.', diagnostics});
        return;
      }
    ...
    try {
      ...
      self.postMessage({ output: result.stdout || 'Program finished with no output.', diagnostics });
    } finally { result.free(); }
  } catch (error) { self.postMessage({ output: String(error?.message ?? error), diagnostics }); }
};
