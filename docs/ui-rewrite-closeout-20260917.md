...
Windows 托管平台已通过 49 项桌面交付检查，包含 Native AOT 链接、搬迁和运行，
Studio VM 也已通过。随后 Studio JIT 在发布暂存目录时遇到 EPERM。桌面项目与
Studio 共用有界目录重命名：仅 Windows 的共享/访问错误可重试，累计等待上限
4.55 秒，支持取消，保留原始错误和已有产物的回滚。新增瞬时占用、永久失败、
目标冲突和取消检查；不改变新项目禁止覆盖已有目录的发布规则。

### 最后跨平台复查与诊断

提交 `01bec364` 的托管检查确认 Windows 桌面 49 项和 Studio 三后端 36 项
全部通过；Linux 新桌面、Web、Rust 和语言检查也通过。macOS 桌面交付
52 项通过，随后 Studio VM 的可选 UI 预览编译出现一次 Wasm 内存越界，
该次失败保留在 [CI 记录](https://github.com/vo-lang/volang/actions/runs/35217412212)。

同一托管 macOS 15 环境的独立编译、并行 Worker、取消后编译和真实 iframe
预览探针均通过。使用实际 Studio 源码、开发配置原生 VM 宿主和 macOS 构建的
发布配置 Wasm 编译器，连续十次完整操作也全部通过，见
[完整 Studio 诊断](https://github.com/vo-lang/volang/actions/runs/35222708886)。
这些结果尚未确认原越界的原因，也不能代替最终提交的生产配置 CI。

本机 macOS 26.6.2 的独立压力探针另外观察到多次连续 Wasm 编译挂起；
托管 macOS 15 的对应探针未复现，尚未确认两者具有相同原因。复用编译器实例
和缓存 Wasm Module 都未消除本机现象，因此未将这些实验方案加入产品。

原检查丢失 Worker 调用栈，并等待预览超时后才报告错误。现保留最多 64 KiB
的调用栈到每次预览独立的诊断状态，界面继续使用简短错误；桌面验收直接报告
该错误。回归覆盖长度限制、界面消息、Worker 清理与旧运行晚到错误的隔离。
补充诊断后的本地 168 项 Node 契约、Studio 三后端 36 项窗口检查与文档验证
全部通过；最终提交继续运行完整托管 CI。

PR #19 的最终提交通过全部 12 项检查并合并为 `e858021a`。随后的主分支
全量检查发现，兼容 VM 页面没有同步 SSR 根节点上的提交版本号，Gallery
输入测试因此持续等待初始版本；应用的输入处理本身已经完成。VM 启动脚本
现从每次成功提交后的实际 adapter revision 更新该属性，原 Gallery 完整
场景从超时恢复为通过。现有组件 smoke 同时验证每次更新、重排、移除和替换
都具有提交回执，使该问题在合并前即可被检测。

旧 Studio 浏览器入口也改为消费其专用 `legacy-studio` 产物。本地已重建并
通过 Gallery、数据、内容、媒体、工作台、旧 Studio、启动、canary 与生命周期
九个完整浏览器场景，覆盖离线编辑、预览、保存、恢复和 Worker 释放。
