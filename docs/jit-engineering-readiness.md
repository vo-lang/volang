...
- VM full-function JIT 与 OSR 的 non-OK `JitResult` handling 共享 bridge transition 层；`Ok` 分支分别适配 full return copy 与 OSR `loop_exit_pc`。legacy-named fallback stats 只记录 semantic side exit，不记录 compile/metadata/internal ABI failure。
...
- `vo-vm::vm::jit::tests::call_kind_fallback_reasons_are_shared_by_full_and_osr_result_handling` 与 OSR panic location tests：full-function 与 OSR non-OK `JitResult` 共享 transition handling，compile/internal errors 不进入 semantic side-exit stats，Panic 仍要求 typed payload 与 location。
