...
## 2026-08-07 shared-code ownership and dynamic dispatch

- `SharedJitCode` 同时负责可执行内存和精确不可变 `LoadedModule` 的生命周期。它先析构 compiler，再析构持有的 module owner；VM 字段顺序不再承担模块生命周期证明。
- full-function 与 OSR 编译统一获取 verified shared compiler guard。相关 manager 若提交内容相同但身份不同的 image，会以 `ModuleScopeChanged` fail closed。
- prepared closure/interface miss 可进入 shadow-safe compiled leaf。interface IC 发布仍限于 frame-elided entry；heap-return 函数不会进入这两种直接返回 ABI。
- 64-bit target 的 `PreparedCall` 为 32 bytes；`DynCallIC` 为 32 bytes。closure callsite 不再分配 IC scratch，interface entry 只匹配精确 owner 与 receiver slot0。
- 当前 release 三次均值：`jit-call` JIT 297.2 ms、VM 586.3 ms；`jit-loop` JIT 43.9 ms、VM 893.7 ms。
...
反序列化边界现在拒绝非法 `SlotType`、`ValueKind`、`RuntimeType`、`ChanDir`、`ExtSlotKind` 和未知 instruction metadata tag。`ValueMeta` / `ValueRttid` raw 值会先验证低 8 位 kind tag，再进入 runtime 对象。
...
- `JumpIf` / `JumpIfNot` / `ForLoop` 的 semantics matrix 必须声明 `LocalSlotLayout` verifier requirement，并且 fail-fast policy 必须覆盖 `LayoutMismatch`。直接一致性测试会阻止 verifier、effects 和 lowering 再次静默漂移。
...
- `vo-jit/src/metadata_contract.rs` 是当前 opcode metadata requirement 的单一事实源。`semantics` 从这里读取 requirement，`verifier` 从这里拒绝 missing/wrong metadata，测试直接核对当前 strict lowering schema。
...
- `vo-jit/src/test_fixtures.rs` 是共享 JIT 测试 builder。默认从 `code` / `slot_types` 派生 `gc_scan_slots`、`borrowed_scan_slots_prefix`、`has_defer`、`has_calls`、`has_call_extern` 和 `instruction_metadata` 长度；需要测试 drift 时，测试必须显式破坏构造后的字段。
...
Static `Call` bytecode 必须通过 `FuncBuilder::emit_static_call` 发射，callee id 只在 instruction 中编码，参数与返回布局只从 callee `FunctionDef` 读取。动态 closure/interface/extern call 通过对应 typed builder 写入 `CallLayout` / `CallExternLayout` metadata。

`FuncBuilder` / wrapper 代码是 instruction 与 `instruction_metadata` 的 typed emit API 边界。loop hint metadata patch 通过 builder 内部 typed patch 方法完成；method-value embedded-interface wrapper 通过 `FuncBuilder::new_closure`、`emit_ptr_get`、`emit_call_iface` 和 dynamic call buffer helper 构造，不再手工按 pc 写 `instruction_metadata`。
...
- `vo-common-core::serialize::tests::malformed_*`、`rejects_instruction_metadata_length_drift_during_deserialize`、`removed_map_metadata_tags_are_rejected`、`rejects_non_current_bytecode_versions`：非法 slot/kind/runtime/metadata tag、current-version metadata 长度漂移、移除的 metadata tag、非当前 bytecode version 都必须报错。
...
- `vo-jit::metadata::tests::metadata_facts_read_expected_instruction_layouts` 与 effects metadata tests：JIT effects/loop analysis 通过 `MetadataFacts` 读取当前需要的 layout facts，不重新引入局部 `InstructionMetadata` decode table。
...
- `vo-codegen::tests::generated_*`：生成的 `FunctionDef` 派生字段、JIT branch condition slot、packed transfer RTTID、static call callee ABI 和 Return ABI。
...
本轮从当前源码重新审计 strict JIT 入口，不以解释器 fallback 或宽松 metadata 接受掩盖错误。覆盖范围包括 `vo-common-core` bytecode/serialize、`vo-jit` verifier/semantics/metadata contract、`vo-vm` strict load/init/dispatch、`vo-engine::RunMode::Jit`、`vo-codegen` metadata 生产、JIT/VM return/call/branch/loop/GC frame 边界，以及 runtime helper/callback ABI manifest。

### 本轮新增架构不变量

- 所有 VM `load` / `load_with_extensions` 都必须在接收 module 前构造 common-verified `LoadedModule`；strict JIT `load` / `try_init_jit` 在加载提交点直接绑定该 certificate、运行一次 JIT 专属 verifier 并初始化唯一 dispatch table。首次热点编译只检查模块身份。`RunMode::Jit` 只能使用 strict 构造入口。
- Best-effort API 只控制 JIT 初始化失败时是否退回解释器；它不能跳过 `ModuleVerifier`，也不能接受非法 bytecode/module。
- Versioned bytecode 只接受当前版本；携带 `instruction_metadata` table 时必须满足 `instruction_metadata.len() == code.len()`。
- 已移除的 map metadata tag 不属于兼容 schema；反序列化或 `ModuleVerifier` 必须拒绝，strict metadata contract 只接受当前 lowering 合同。
- Semantics、verifier、effects 和 capability 对 layout-sensitive control flow 必须通过直接一致性测试自证：`JumpIf`、`JumpIfNot`、`ForLoop` 读取 `Value` slot，声明 `LocalSlotLayout`，并在 `LayoutMismatch` 上 fail fast。
...
- Static `Call` 的 callee、arg slots、ret slots 以 callee `FunctionDef` 为唯一 ABI authority；已退役的 shape 字段必须为零。
...
- Codegen 生成 `__island_init__` / `__entry__` 调用 `__init__`、用户 init 和 `main` 时，必须以 callee `FunctionDef` 为 ABI authority 分配 discard return buffer；入口函数不能用调用点自带的返回宽度掩盖 callee 返回值。
...
- Serializer metadata 长度漂移：`rejects_instruction_metadata_length_drift_during_deserialize` 修复前会接受 current-version module 的空 metadata table，修复后反序列化返回 `InvalidInstructionMetadata`。
...
- Static call 双重布局来源：旧实现允许调用点携带 compact shape；当前 schema 删除该布局来源，verifier 要求退役字段为零，并只读取 callee `FunctionDef`。
...
- Removed metadata schema 漂移：`removed_map_metadata_tags_are_rejected` 与 metadata contract 测试修复前会放过已移除的 map metadata tag，修复后旧 tag/旧版本在反序列化或 VM 共享验证边界被拒绝。
...
- Entry static call ABI：全量 `vo-test-osr` 曾暴露入口函数没有按 returning `main` 分配 discard buffer；修复后入口调用直接按 callee return layout 分配缓冲，指令不再携带返回宽度副本。
...
| JIT verifier/contract/effects/lowering unit | `cargo test -p vo-jit --locked` | 通过，256 passed |
| codegen metadata/function layout | `cargo test -p vo-codegen` | 通过，14 unit + 56 integration，1 doctest ignored |
| VM strict JIT bridge/callback/materialization | `cargo test -p vo-vm --features jit --locked` | 通过，726 passed |
| engine `RunMode::Jit` strict errors/trap parity | `cargo test -p vo-engine --locked` | 通过，234 passed |
| no-JIT feature boundary | `cargo test -p vo --no-default-features --locked` | 通过，34 passed；依赖树不含 `vo-jit` |
| OSR gate 与 manifest contract | `cargo test -p vo-test --locked` / `cargo test -p vo-dev --locked` | 通过，15 + 93 passed |
| formatting/hygiene | `cargo fmt --all -- --check` / `git diff --check` | 通过 |
| repo language VM/JIT/OSR differential | `cargo run -q -p vo-dev --locked -- test run --suite lang --targets native --jobs 4` | 通过，3508 passed |
| dedicated OSR entry contract | `cargo run -q -p vo-dev --locked -- test run --suite lang --matrix osr-contract` | 通过，15 passed；5 个 OSR candidate 均真实进入 loop JIT |
| GC-stress VM/JIT differential | `./d.py test gc -j4` | 通过，42 passed |
| language strict contract | `tests/lang/cases/bugs/2026_06_01_jit_strict_contract_boundaries.vo` | 通过，由 native differential 覆盖 |
| context/time JIT timing robustness | `tests/lang/cases/stdlib/context.vo`、`tests/lang/cases/stdlib/time.vo` | 通过，由 native differential 覆盖；短超时过期路径保持独立覆盖 |
