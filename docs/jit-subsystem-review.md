...
合法 side-exit 清单：cold/not-hot interpreter execution、WaitIo/WaitQueue/Yield/Replay、regular/prepared call VM materialization、OSR normal exit、direct-call stack-capacity trampoline、以及显式命名的 best-effort embedding API。这些 side exit 是调度、host I/O、stack capacity 或 embedding 语义边界，不是 JIT 编译失败的静默吞错。
...
本轮后续重构没有移除已有 JIT opcode 能力；唯一收紧的是 `has_defer` 的自递归 direct-native call 优化：这类调用现在走 VM call materialization，以保证 defer/recover 依赖的真实 `CallFrame` 深度不被 native shadow call chain 隐藏。函数本身仍可 JIT，降级的是不安全的 direct-call 快路径。
...
- source behavior：新增 `tests/lang/cases/bugs/2026_05_28_jit_recursive_defer_fallback.vo`，并保留既有 WaitIo、direct-call materialization、nested JIT calls、panic/recover、loop defer/OSR cases。
...
- VM/JIT frame/runtime boundary：`vo-vm --features jit` 覆盖 side-exit stats、nested materialization invariant、borrowed-parent materialization invariant；`vo-runtime` 覆盖 JIT runtime symbol/ABI manifests。
...
本轮修复不移除 JIT 能力。遇到 capacity 不足或动态 IC fast path 不满足条件时，选择 side-exit 到已有 VM materialization 路径；遇到控制流常量不确定时，仅关闭该点的常量优化，不把函数整体踢出 JIT。
