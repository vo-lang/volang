...
This document preserves the previous framework overview and its declared
product baseline. New Web applications start with [the current UI guide](README.md).
The declarations below apply to the retained compatibility implementation;
the Web replacement has separate acceptance and evidence. Existing native
commands and source APIs remain available during migration.
...
the VM or JIT, and release through Wasm VM or Native AOT. Application
...
- shared layout, paint, text, IME, focus, accessibility, and system-service
  contracts;
- browser VM development, Wasm VM release, native VM/JIT development,
...
and Wasm VM. The permanent authored probe certifies move, removal,
...
Build a static Wasm VM application:
...
New applications use `vo ui create` and the [replacement templates](next/toolchain.md).
Existing applications follow the [migration guide](next/guides/migration.md).
