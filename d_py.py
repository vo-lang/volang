...
STUDIO_EXAMPLES_DIR = PROJECT_ROOT / 'examples'
STUDIO_EXAMPLES_GENERATED_FILE = PROJECT_ROOT / 'studio' / 'src' / 'generated' / 'examples.generated.ts'
...
def sync_studio_examples_file():
    """Generate studio/src/generated/examples.generated.ts from repo examples/*.vo."""
    if not STUDIO_EXAMPLES_DIR.exists():
        print(f"{Colors.RED}Examples directory not found: {STUDIO_EXAMPLES_DIR}{Colors.NC}")
        sys.exit(1)

    entries = []
    for path in sorted(STUDIO_EXAMPLES_DIR.rglob('*.vo')):
        rel_path = path.relative_to(STUDIO_EXAMPLES_DIR).as_posix()
    ...
    for rel_path, content in entries:
        lines.append(
            f"  {{ path: {json.dumps(rel_path)}, content: {json.dumps(content)} }},"
        )
    lines.append('];')
    lines.append('')
    generated = '\n'.join(lines)

    STUDIO_EXAMPLES_GENERATED_FILE.parent.mkdir(parents=True, exist_ok=True)
    previous = None
    if STUDIO_EXAMPLES_GENERATED_FILE.exists():
        previous = STUDIO_EXAMPLES_GENERATED_FILE.read_text(encoding='utf-8')
    if previous != generated:
        STUDIO_EXAMPLES_GENERATED_FILE.write_text(generated, encoding='utf-8')
        print(f"{Colors.GREEN}✓ studio examples synced:{Colors.NC} {len(entries)} files")
    else:
        print(f"{Colors.DIM}studio examples up-to-date{Colors.NC}")
