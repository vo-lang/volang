...
function runStaticSmoke() {
  ...
  assert(existsSync(join(artifactRoot, 'github.com@vo-lang@voplay/v0.1.27/voplay_island.js')), 'missing voplay quickplay JS artifact');
  assert(existsSync(join(artifactRoot, 'github.com@vo-lang@voplay/v0.1.27/voplay_island_bg.wasm')), 'missing voplay quickplay WASM artifact');
...
async function runHttpSmoke(baseUrl, buildId) {
  ...
  assert(script.includes('v0.1.27'), 'entry bundle does not reference current voplay quickplay artifacts');
  ...
  assert(voplay?.version === 'v0.1.27', 'remote deps manifest does not use voplay v0.1.27');
