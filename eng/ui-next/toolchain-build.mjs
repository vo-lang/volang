...
export async function buildToolchain(directory,{signal,compiler=toolchain.compiler,desktop} = {}) {
  ...
  try {
    ...
    await writeFile(join(stage,'README.md'),[
      ...
      'Add this package’s bin directory to PATH, then run `vo ui verify` and `vo ui create /path/to/my-app`. Inside the project, run `vo ui dev`; check, build, preview and test use the same shape. Project commands also accept a directory argument or explicit `--project /path/to/my-app`. Direct `node ui.mjs` commands use the explicit `--project` argument. Optional editor and plot libraries are bundled only for projects declaring them.',
