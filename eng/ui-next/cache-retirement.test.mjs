...
import {retireAssetCache} from '../../apps/studio/next/cache-retirement.js';

async function workerFixture(fail=false) {
  const events=new Map(),actions=[];
  runInNewContext(await readFile(new URL('../../apps/studio/next/cache-retirement-worker.js',import.meta.url),'utf8'),{
...
test('Studio updates the known root registration without registering another worker',async()=>{
  ...
  await retireAssetCache(window);assert.equal(updates,1);
  registration.scope=origin+'/another/';await retireAssetCache(window);
  registration.scope=origin+'/';registration.active.scriptURL=origin+'/other-worker.js';await retireAssetCache(window);
  await retireAssetCache({navigator:{},location:{origin}});
...
test('retirement requests activation after installation and releases its listener',async()=>{
  for (const initial of ['installing','installed','activated','redundant']) {
    ...
    await retireAssetCache({location:{origin},navigator:{serviceWorker:{getRegistration:async()=>registration}}});
