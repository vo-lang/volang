...
//! - Function declarations (Vo + runtime)
//! - Constant table
//! - Type metadata
...
use vo_vm::bytecode::{Constant, FunctionDef, Module as BytecodeModule};
...
pub struct CompileContext<'a> {
    /// Vo function IDs (bytecode index -> Cranelift FuncId)
...
impl<'a> CompileContext<'a> {
    ...
    /// Declare all Vo functions in the module.
    ...
    pub fn declare_all_functions<M: Module>(&mut self, module: &mut M) -> Result<()> {
        for (idx, func_def) in self.bytecode.functions.iter().enumerate() {
            let func_id = self.declare_vo_function(module, idx as u32, func_def)?;
            self.vo_funcs.insert(idx as u32, func_id);
        }
        Ok(())
    }

    /// Declare a single Vo function.
    fn declare_vo_function<M: Module>(
        &self,
        module: &mut M,
        _idx: u32,
        ...
        let name = format!("vo_{}", func_def.name);
        let func_id = module.declare_function(&name, Linkage::Local, &sig)?;
        
        Ok(func_id)
    }

    /// Get a declared Vo function by bytecode index.
    pub fn get_vo_func(&self, idx: u32) -> Option<FuncId> {
        self.vo_funcs.get(&idx).copied()
    }
