...
pub use vo_common_core::SlotType;

/// Magic bytes for bytecode files.
pub const MAGIC: [u8; 4] = *b"VOBJ";
...
/// Extern function definition (functions called from Vo to outside).
#[derive(Clone, Debug)]
...
fn read_type_meta(cursor: &mut Cursor<&[u8]>) -> Result<TypeMeta, BytecodeError> {
    let value_kind = vo_common_core::ValueKind::from_u8(read_u8(cursor)?);
