...
use vo_runtime_core::objects::channel::{ChannelState, SendResult, RecvResult};
...
#[no_mangle]
pub unsafe extern "C" fn vo_chan_new(elem_type: u32, capacity: u64) -> GcRef {
    use vo_runtime_core::objects::channel;
...
#[no_mangle]
pub unsafe extern "C" fn vo_chan_send(chan_ref: GcRef, value: u64) -> u8 {
    let scheduler = match current_scheduler() {
        Some(s) => s,
        None => return 0,
    };
...
#[no_mangle]
pub unsafe extern "C" fn vo_chan_recv(chan_ref: GcRef, ok: *mut u8) -> u64 {
    let scheduler = match current_scheduler() {
        Some(s) => s,
        None => {
            if !ok.is_null() { *ok = 0; }
...
/// Close a channel.
#[no_mangle]
pub unsafe extern "C" fn vo_chan_close(chan_ref: GcRef) {
    let scheduler = match current_scheduler() {
        Some(s) => s,
        None => return,
    };
...
#[no_mangle]
pub unsafe extern "C" fn vo_go_spawn(func_ptr: u64, args_ptr: *const u64, arg_count: u64) {
    let scheduler = match current_scheduler() {
        Some(s) => s,
        None => return,
    };
...
#[no_mangle]
pub unsafe extern "C" fn vo_defer_push(func_ptr: u64, args_ptr: *const u64, arg_count: u64) {
    let args: Vec<u64> = if !args_ptr.is_null() && arg_count > 0 {
        std::slice::from_raw_parts(args_ptr, arg_count as usize).to_vec()
    } else {
        Vec::new()
...
#[no_mangle]
pub unsafe extern "C" fn vo_defer_pop() {
...
#[no_mangle]
pub unsafe extern "C" fn vo_panic(value: u64) {
    // Set panic value
    PANIC_VALUE.with(|pv| {
        *pv.get() = Some(value);
    });
...
#[no_mangle]
pub unsafe extern "C" fn vo_recover() -> u64 {
    PANIC_VALUE.with(|pv| {
...
#[no_mangle]
pub unsafe extern "C" fn vo_select_start(case_count: u64, has_default: u64) {
...
#[no_mangle]
pub unsafe extern "C" fn vo_select_add_send(chan: u64, value: u64) {
    SELECT_STATE.with(|state| {
...
/// happens in vo_select_exec based on which case is chosen.
#[no_mangle]
pub unsafe extern "C" fn vo_select_add_recv(chan: u64) -> (u64, u64) {
    SELECT_STATE.with(|state| {
        if let Some(ref mut s) = *state.get() {
...
#[no_mangle]
pub unsafe extern "C" fn vo_select_exec() -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn vo_iter_begin(container: u64, iter_type: u64) -> u64 {
...
#[no_mangle]
pub unsafe extern "C" fn vo_iter_next(_handle: u64, out: *mut u64) {
    use vo_runtime_core::gc::Gc;
    ...
    let (done, key, value) = ITER_STACK.with(|stack| {
        ...
        match state {
            ...
            IterStateJit::String { str_ref, byte_pos } => {
                ...
                let str_gcref = *str_ref as vo_runtime_core::gc::GcRef;
                ...
                if *byte_pos >= bytes.len() {
                    ...
                    let (rune, width) = vo_common_core::utf8::decode_rune(&bytes[pos..]);
                    *byte_pos += width;
...
#[no_mangle]
pub unsafe extern "C" fn vo_iter_end(_handle: u64) {
    ITER_STACK.with(|stack| {
        let stack = &mut *stack.get();
        stack.pop();
    });
...
mod tests {
    ...
    fn test_defer_basic() {
        ...
        unsafe {
            ...
            vo_defer_push(increment as u64, args.as_ptr(), 1);
    ...
    fn test_defer_lifo_order() {
        ...
        unsafe {
            ...
            // Result: ((0 + 2) + 1) * 10 = 30
            vo_defer_push(mul_10 as u64, args.as_ptr(), 1);
            vo_defer_push(add_1 as u64, args.as_ptr(), 1);
            vo_defer_push(add_2 as u64, args.as_ptr(), 1);
