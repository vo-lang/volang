...
impl FileSet {
    ...
    /// Collect all .gox files from a directory recursively.
    ...
    fn collect_dir<F: FileSystem>(&mut self, fs: &F, dir: &Path) -> io::Result<()> {
        for entry in fs.read_dir(dir)? {
            ...
            } else if entry.extension().map_or(false, |e| e == "gox") {
...
mod tests {
    ...
    fn test_memory_fs() {
        let fs = MemoryFs::new()
            .with_file("/project/main.gox", "package main")
            .with_file("/project/lib/util.gox", "package lib");
        
        assert!(fs.exists(Path::new("/project/main.gox")));
        assert!(fs.exists(Path::new("/project/lib")));
        assert!(fs.is_dir(Path::new("/project/lib")));
        assert!(!fs.is_dir(Path::new("/project/main.gox")));
        
        let content = fs.read_file(Path::new("/project/main.gox")).unwrap();
    ...
    fn test_file_set_collect() {
        let fs = MemoryFs::new()
            .with_file("/project/main.gox", "package main")
            .with_file("/project/lib/util.gox", "package lib")
            .with_file("/project/lib/helper.gox", "package lib")
            .with_file("/project/readme.md", "# Readme");
        
        let file_set = FileSet::collect(&fs, Path::new("/project")).unwrap();
        
        // Should only collect .gox files
        assert_eq!(file_set.files.len(), 3);
        assert!(file_set.files.contains_key(Path::new("/project/main.gox")));
        assert!(file_set.files.contains_key(Path::new("/project/lib/util.gox")));
