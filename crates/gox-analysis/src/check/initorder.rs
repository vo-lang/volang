                ...
                if self.lobj(*o).entity_type().is_func() {
                    for p in node.pred.borrow().iter() {
                        if p != o {
                            for s in node.succ.borrow().iter() {
                                if s != o {
                                    if let Some(edge) = map.get(p) {
                                        edge.succ.borrow_mut().insert(*s);
                                    }
                                    if let Some(edge) = map.get(s) {
                                        edge.pred.borrow_mut().insert(*p);
                                        edge.pred.borrow_mut().remove(o);
                                    }
                                }
                            }
                            if let Some(edge) = map.get(p) {
                                edge.succ.borrow_mut().remove(o);
                            }
                        }
                    }
                    None
...
fn find_path(
    ...
    let decl_key = map.get(&from)?;
    let decl = &tc_objs.decls[*decl_key];
