...
//!    `declare_var_needs_stack_map()` (in vo-codegen-cranelift)
//!
...
//!
//! 4. Stack maps are registered with `vo_runtime_native::stack_map::register_stack_map()`
//!
...
use vo_runtime_native::stack_map::{StackMapEntry, register_stack_map};
...
impl JitCompiler {
    ...
    pub fn compile_module(&mut self, bytecode: &BytecodeModule) -> Result<()> {
        ...
        let slot_types_data: Vec<Vec<_>> = bytecode.struct_types.iter()
            ...
            .collect();
        vo_runtime_core::gc_types::init_struct_slot_types(slot_types_data);
        ...
        vo_runtime_native::init_globals(globals_is_ref.len(), globals_is_ref);
