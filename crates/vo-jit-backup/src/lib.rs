...
pub mod loop_compiler;

pub use func_compiler::FunctionCompiler;
pub use translator::BytecodeTranslator;
pub use gc_tracking::{GcRefTracker, StackMap};
pub use loop_analysis::LoopInfo;
pub use loop_compiler::{LoopCompiler, LoopFunc, CompiledLoop, LOOP_RESULT_PANIC};
...
pub struct JitCache {
    ...
    loops: HashMap<(u32, usize), CompiledLoop>,  // (func_id, begin_pc) -> loop
}
...
impl JitCache {
    ...
    /// Get the loop function pointer.
    ///
    /// # Safety
    /// The returned function pointer must only be called with valid JitContext.
    pub unsafe fn get_loop_func_ptr(&self, func_id: u32, begin_pc: usize) -> Option<LoopFunc> {
        self.loops.get(&(func_id, begin_pc)).map(|l| {
            std::mem::transmute(l.code_ptr)
        })
    }
}
...
impl JitCompiler {
    ...
    pub fn compile(
        ...
        let mut func_ctx = FunctionBuilderContext::new();
        {
            ...
            let str_funcs = crate::func_compiler::StringFuncs {
            ...
            let map_funcs = crate::func_compiler::MapFuncs {
            ...
            let array_funcs = crate::func_compiler::ArrayFuncs {
            ...
            let slice_funcs = crate::func_compiler::SliceFuncs {
            ...
            let misc_funcs = crate::func_compiler::MiscFuncs {
    ...
    pub fn compile_loop(
        ...
        // 1. Create loop function signature: (ctx: *mut, locals: *mut) -> u32
        let ptr_type = self.module.target_config().pointer_type();
        let mut sig = Signature::new(self.module.target_config().default_call_conv);
        sig.params.push(AbiParam::new(ptr_type)); // ctx
        sig.params.push(AbiParam::new(ptr_type)); // locals
        sig.returns.push(AbiParam::new(types::I32)); // exit_pc
        ...
        // 4. Build loop IR using LoopCompiler
        let mut func_ctx = FunctionBuilderContext::new();
        let stack_map = {
            let safepoint_func = self.module.declare_func_in_func(self.helper_funcs.safepoint, &mut self.ctx.func);
            let call_vm_func = self.module.declare_func_in_func(self.helper_funcs.call_vm, &mut self.ctx.func);
        ...
        // Debug: print Cranelift IR if VO_DUMP_JIT_IR is set
        if std::env::var("VO_DUMP_JIT_IR").is_ok() {
            eprintln!("=== Loop JIT IR for func_{} loop@{} ({}) ===", func_id, begin_pc, &func_name);
            eprintln!("{}", self.ctx.func);
            eprintln!("=== End IR ===\n");
        }
