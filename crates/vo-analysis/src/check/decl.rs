...
impl Checker {
    ...
    pub(crate) fn decl_stmt(&mut self, decl: &vo_syntax::ast::Decl) {
        use vo_syntax::ast::Decl;

        match decl {
            Decl::Const(cdecl) => {
                let mut last_full_spec: Option<&vo_syntax::ast::ConstSpec> = None;
    ...
    fn arity_match_const(
        &self,
        spec: &vo_syntax::ast::ConstSpec,
        init_spec: Option<&vo_syntax::ast::ConstSpec>,
    ) {
    ...
    fn arity_match_var(&self, spec: &vo_syntax::ast::VarSpec) {
        let l = spec.names.len();
