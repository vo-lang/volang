...
//!
//! Translates Vo bytecode to Cranelift IR with support for:
//! - Control flow (jumps, branches)
//! - Function calls (Vo and runtime functions)
...
//!   - Reload values after safepoints (to support moving GC)
//!
//! ## TODO: Runtime Stack Scanning
//!
//! To complete native stack GC scanning:
//! 1. Extract stack maps from CompiledCode after compilation
//! 2. Store mapping: return_address -> stack_map
//! 3. During GC, walk the native call stack
//! 4. For each frame, lookup stack map by return address
//! 5. Read GC refs from stack slots specified in stack map
...
impl FunctionTranslator {
    ...
    pub fn translate<M: Module>(
        ...
        module: &mut M,
        slot_types: &[vo_vm::bytecode::SlotType],
    ) -> Result<()> {
    ...
    fn declare_variables(&mut self, builder: &mut FunctionBuilder, slot_types: &[vo_vm::bytecode::SlotType]) {
        use vo_vm::bytecode::SlotType;
    ...
    /// Get or import a Vo function reference.
    fn get_vo_func_ref<M: Module>(
        &mut self,
        ...
        let func_id = ctx.get_vo_func(func_idx)
    ...
    fn translate_instruction<M: Module>(
        ...
        match inst.opcode() {
            ...
            Opcode::Call => {
                ...
                let func_ref = self.get_vo_func_ref(builder, module, ctx, func_idx)?;
            ...
            Opcode::StrConcat => {
                // vo_rt_string_concat(type_id, a, b) -> GcRef
            ...
            Opcode::Go => {
                ...
                let target_func_ref = self.get_vo_func_ref(builder, module, ctx, func_idx)?;
