...
/// Infer VarKind and optional type symbol for an expression.
/// Uses the type checker's expr_types when available, with comprehensive fallbacks.
fn infer_var_kind_and_type(ctx: &CodegenContext, fctx: Option<&FuncContext>, expr: &gox_syntax::ast::Expr) -> (crate::context::VarKind, Option<gox_common::Symbol>) {
    ...
    // First try type checker's expr_types
    let (kind, type_sym) = ctx.lookup_expr_var_kind(expr);
    if kind != VarKind::Other {
        return (kind, type_sym);
    }
    
    // Fallback: syntactic analysis for cases not covered by expr_types
    match &expr.kind {
        ExprKind::CompositeLit(lit) => {
            match &lit.ty.kind {
                ...
                TypeExprKind::Ident(ident) => {
                    ...
                    if expr::lookup_named_type(ctx, ident.symbol).map_or(false, |ty| matches!(ty, Type::Obx(_))) {
...
/// Uses the type checker's expr_types for accurate type information.
pub fn infer_runtime_type_id(ctx: &CodegenContext, _fctx: &FuncContext, expr: &gox_syntax::ast::Expr) -> u16 {
    ctx.lookup_expr_runtime_type_id(expr)
}
...
/// Uses the type checker's expr_types for accurate type information,
/// with fallback to local variable lookup for identifiers.
fn is_map_expr(ctx: &CodegenContext, fctx: &FuncContext, expr: &gox_syntax::ast::Expr) -> bool {
    use gox_analysis::Type;
    use gox_syntax::ast::ExprKind;
    
    // First try type checker's expr_types
    if let Some(ty) = ctx.lookup_expr_type(expr) {
        return matches!(ty, Type::Map(_));
    }
    
    // Fallback: Check local variable for identifiers
...
fn compile_for(
    ...
    match &for_stmt.clause {
        ...
        ForClause::Range { key, value, define, expr: range_expr } => {
            ...
            let iter_type = if is_map_expr(ctx, fctx, range_expr) {
                1u16 // map
            } else {
                0u16 // slice (default)
            };
