...
impl<'a> CodegenContext<'a> {
    ...
    /// Convert gox_analysis::Type directly to TypeTag.
    /// This combines type_to_var_kind and var_kind_to_type_tag for efficiency.
    pub fn type_to_type_tag(&self, ty: &gox_analysis::Type) -> gox_vm::ffi::TypeTag {
        use gox_analysis::{Type, BasicType, UntypedKind};
        use gox_vm::ffi::TypeTag;
        
        match ty {
            Type::Basic(BasicType::String) | Type::Untyped(UntypedKind::String) => TypeTag::String,
            Type::Basic(BasicType::Float64) | Type::Basic(BasicType::Float32) |
            Type::Untyped(UntypedKind::Float) => TypeTag::Float64,
            Type::Basic(BasicType::Bool) | Type::Untyped(UntypedKind::Bool) => TypeTag::Bool,
            Type::Basic(BasicType::Int32) => TypeTag::Int32, // for rune
            Type::Nil => TypeTag::Nil,
            _ => TypeTag::Int64,
        }
    }
    
    /// Look up the type tag for any expression using the type checker's expr_types.
    pub fn lookup_expr_type_tag(&self, expr: &gox_syntax::ast::Expr) -> gox_vm::ffi::TypeTag {
        use gox_vm::ffi::TypeTag;
        if let Some(ty) = self.lookup_expr_type(expr) {
            self.type_to_type_tag(&ty)
        } else {
            TypeTag::Int64 // default fallback
        }
    }
    
    /// Extract the type symbol (name) from a Type if it's a named type.
    pub fn type_to_type_sym(&self, ty: &gox_analysis::Type) -> Option<gox_common::Symbol> {
        use gox_analysis::Type;
        match ty {
            Type::Named(id) => {
                self.result.named_types.get(id.0 as usize).map(|info| info.name)
            }
            _ => None
        }
    }
    
    /// Look up VarKind and optional type symbol for an expression.
    /// This is the unified lookup for variable declarations.
    pub fn lookup_expr_var_kind(&self, expr: &gox_syntax::ast::Expr) -> (VarKind, Option<gox_common::Symbol>) {
        if let Some(ty) = self.lookup_expr_type(expr) {
            let kind = self.type_to_var_kind(&ty);
            let type_sym = self.type_to_type_sym(&ty);
            (kind, type_sym)
        } else {
            (VarKind::Other, None)
        }
    ...
    /// Convert a Type to runtime builtin type ID.
    pub fn type_to_builtin_type_id(&self, ty: &gox_analysis::Type) -> u16 {
        use gox_analysis::{Type, BasicType, UntypedKind};
        use gox_vm::types::builtin;
        
        match ty {
            Type::Basic(BasicType::String) | Type::Untyped(UntypedKind::String) => builtin::STRING as u16,
            Type::Basic(BasicType::Float64) | Type::Basic(BasicType::Float32) |
            Type::Untyped(UntypedKind::Float) => builtin::FLOAT64 as u16,
            Type::Basic(BasicType::Int32) => builtin::INT32 as u16, // rune
            Type::Slice(_) => builtin::SLICE as u16,
            Type::Map(_) => builtin::MAP as u16,
            Type::Named(id) => {
                // Named type - compute runtime type_id
                // Skip interface types in the count since they don't have runtime type_ids
                let mut concrete_idx = 0u32;
                for (i, info) in self.result.named_types.iter().enumerate() {
                    if matches!(info.underlying, Type::Interface(_)) {
                        continue;
                    }
                    if i == id.0 as usize {
                        return (builtin::FIRST_USER_TYPE + concrete_idx) as u16;
                    }
                    concrete_idx += 1;
                }
    ...
    /// Look up runtime type ID for an expression.
    pub fn lookup_expr_runtime_type_id(&self, expr: &gox_syntax::ast::Expr) -> u16 {
        use gox_vm::types::builtin;
        if let Some(ty) = self.lookup_expr_type(expr) {
            self.type_to_builtin_type_id(&ty)
        } else {
            builtin::INT64 as u16
        }
    }
