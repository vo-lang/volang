...
/// Infer the FFI TypeTag for an expression.
/// Uses the type checker's expr_types when available, with fallback to syntactic analysis.
pub fn infer_type_tag(ctx: &CodegenContext, fctx: &FuncContext, expr: &Expr) -> TypeTag {
    ...
    if let Some(ty) = ctx.lookup_expr_type(expr) {
        // Got a type from type checker - convert to TypeTag
        return ctx.type_to_type_tag(&ty);
    }
    
    // Fallback: syntactic analysis
    match &expr.kind {
...
pub fn is_float_expr(ctx: &CodegenContext, fctx: &FuncContext, expr: &gox_syntax::ast::Expr) -> bool {
    ...
    // Fallback: Check local variable or binary expression
    match &expr.kind {
...
pub fn is_string_expr(ctx: &CodegenContext, fctx: &FuncContext, expr: &gox_syntax::ast::Expr) -> bool {
    ...
    // Fallback: Check local variable or literals
    match &expr.kind {
        ...
        ExprKind::Binary(bin) if bin.op == gox_syntax::ast::BinaryOp::Add => {
...
fn compile_call(
    ...
    if let ExprKind::Selector(sel) = &call.func.kind {
        ...
        if let Some(recv_type) = ctx.lookup_expr_type(&sel.expr) {
...
pub fn is_map_expr_with_ctx(ctx: &CodegenContext, fctx: &FuncContext, expr: &gox_syntax::ast::Expr) -> bool {
    ...
    // Fallback: Check local variable for identifiers (backward compatible)
    if let ExprKind::Ident(ident) = &expr.kind {
        if let Some(local) = fctx.lookup_local(ident.symbol) {
            return local.kind == crate::context::VarKind::Map;
        }
    }
...
fn compile_selector(
    ...
    if let Some(ty) = ctx.lookup_expr_type(&sel.expr) {
