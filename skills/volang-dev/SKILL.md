...
- UI: read `ui/AGENTS.md` and `ui/docs/architecture.md`. `ui/next` owns typed Vo component and rendering contracts. `vo-ui-bridge` connects VM exchange; `vo-ui-native`, `vo-ui-webview`, and `vo-ui-desktop-runtime` own native execution and system WebView delivery. Browser hosts live in `vo-web/js/ui_next`; `eng/ui-next` owns tools and acceptance.
