...
- UI: read `ui/AGENTS.md` and `ui/docs/architecture.md`. `ui/next` owns typed Vo component and rendering contracts. `vo-ui-bridge` connects VM exchange; its optional `toolchain` feature composes provider registration for CLI/SSR/AOT; `vo-ui-native`, `vo-ui-webview`, and `vo-ui-desktop-runtime` own native execution and system WebView delivery. Browser hosts live in `vo-web/js/ui_next`; `eng/ui-next` owns tools and acceptance.
- Web and Studio: `vo-web` owns browser VM compilation/execution and VFS; `vo-web/runtime-wasm` supplies Wasm VM host providers. `apps/studio/next` owns Gallery, Docs and Playground. Check `lang/crates/vo-web/package.json` before selecting Web scripts.
...
- Web execution uses the shared VM/runtime compiled to WebAssembly and verified portable bytecode. For memory, extern, scheduling, or UI changes, check both the Wasm runtime and `vo-web/js` host behavior. Native JIT and Native AOT retain their generated-code contracts.
...
Apply the SpanHeap rules to `vo-runtime`-based VM/JIT/Native AOT and Wasm VM work. Wasm VM uses the shared collector, allocator and precise-root contracts.
...
- Wasm VM admission in `vo-web/src/vm.rs` uses 64 KiB pages, pre-grows `current + reserve`, and verifies `current + hard_limit` against the declared maximum.
...
- Maintained language documentation lives in `lang/docs/guides`, `lang/docs/spec`, and `lang/docs/vo-for-gophers.md`; `lang/docs/catalog.toml` selects Studio content, including UI pages. `apps/studio/next/documentation` is generated by `vo-dev generate studio-docs --write`; `vo-dev lint docs` checks it. App protocol outputs use `vo-dev generate app-protocol --write|--check`.
...
- A `native` matrix selects VM/JIT/OSR; it does not establish Native AOT coverage. Use relevant AOT crate/engine tests and build/run probes from CI for Native AOT. Select `wasm` for browser VM language regressions.
...
- For UI changes, use `ui/certification.toml` and `eng/ui-next` contracts to select browser, platform, lifecycle and distribution checks. `ui-certify --check` establishes `declaration-valid`; `product-certified` requires `--evidence <ci-bundle>` with verified complete platform evidence.
