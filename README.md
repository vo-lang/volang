...
- **Run in the browser** — First-class WASM target. Browser execution uses the
  `vo-web` Wasm VM path; native CLI sessions use native VM/JIT/AOT paths.
...
| Wasm VM | Alpha | Verified bytecode execution in browser and Node hosts |
...
**Wasm VM** — runs verified bytecode in the shared runtime compiled to WebAssembly. `vo build --kind=bytecode --target=wasm32-unknown-unknown` emits the application module; `vo-web` supplies the VM and portable browser providers. See the [execution guide](lang/docs/guides/execution-backends.md).
...
Wasm VM run executed end to end through the current `vo-web` runtime under Node. Every 64-bit run also includes JIT and
cache-disabled native AOT. The runner checks that every included Vo backend
produces identical output and records native executable build
...
Check the source and produce a deployable Web directory:
